...
func TestTypePropertyNameOnlyEntryResolvesThroughTheSeam(t *testing.T) {
	for _, name := range []string{"Cooking Time", "作業内容", "C++", "50% done"} {
		t.Run(name, func(t *testing.T) {
			...
			doc := []byte(fmt.Sprintf(`{"formatVersion":"2.0","kind":"object_type","id":"o1","internal_key":"recipe",`+
				`"type_settings":{"layout":"basic","property_definitions":%s},`+
				`"properties":{"Name":"Recipe"}}`, defs))
			require.NoError(t, Validate(doc, Options{}), "I1/I2: the document is valid")
	...
	for _, tc := range []struct{ name, why string }{
		...
		t.Run(tc.why, func(t *testing.T) {
			...
			for surface, err := range map[string]error{
				"full validation": Validate(doc, Options{}),
				"authoring":       ValidateAuthoring(doc),
			} {
				require.Error(t, err, "%s: %s", surface, tc.why)
				assert.Contains(t, err.Error(), typePropertyDefinitionsPath+"/0/"+memberProperty,
					"%s: the refusal points at the conceptual identity slot", surface)
			}
			_, _, err = Unmarshal(doc, Options{GenerateId: seqIds("g")})
			require.Error(t, err, tc.why)
			assert.Contains(t, err.Error(), typePropertyDefinitionsPath+"/0/"+memberProperty,
				"the refusal points at the conceptual identity slot")

			// JSON Schema cannot count the NFC-normalized identity. It keeps
			// the expressible empty/control checks, while the public authoring
			// reader above owns the effective length bound.
			if tc.name == long {
				require.NoError(t, authoringOnly(doc), tc.why)
			} else {
				require.Error(t, authoringOnly(doc), tc.why)
			}
		})
	}
}
