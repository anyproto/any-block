...
// keyadmission_test.go — what the import surface admits: the property-key
// admission rule (import refuses exactly what export strips, checked on the
// stored key a spelling resolves to rather than on the raw document
// spelling), and the two neighbouring admission surfaces the same pass
// closed — the representable date range and the one-id-domain rule. The two
// invariants these are instances of live in flat_invariants_test.go; each
// case here is hand-written so the fixture can express the failure.
...
// `customDate = 1751791445000` is milliseconds stored where seconds
...
func TestExport_SanitizedColumnIdCannotTakeASiblingsId(t *testing.T) {
	...
	data, err := Marshal(model.SmartBlockType_Page, snap, Options{})
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "duplicate id across two surfaces:\n%s", data)
...
//
// The rule was first raised against the refs labeller, which is deleted (§9a).
...
func TestValidate_DerivedCellIdIsClaimedEvenWhenTheCellIsAbsent(t *testing.T) {
	...
	require.NoError(t, Validate([]byte(`{"formatVersion": "2.0", "blocks": [
		{"type": "paragraph", "id": "r1-c2", "text": "x"}]}`), Options{}))
}
...
// ---- Property-key admission control ----
//
...
// vocabulary.LocalAndDerivedRelationKeys, import took them. Among them are the keys
...
func TestValidate_ImportRefusesWhatExportStrips(t *testing.T) {
	...
	for key := range strippedDetailKeys() {
		...
		refused++
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "id": "obj1", "properties": {%q: "x"}}`, key)
		err := Validate([]byte(doc), Options{})
...
// two of them need naming by hand is that vocabulary.LocalAndDerivedRelationKeys,
...
func TestValidate_ResolutionVectorPropertiesRefused(t *testing.T) {
	...
	for key, wantOnList := range map[string]bool{
		...
		assert.True(t, vocabulary.HasRelation(domain.RelationKey(key)),
			"%s is a bundled relation", key)
		assert.Equal(t, wantOnList, onList[key],
			"%s in vocabulary.LocalAndDerivedRelationKeys", key)
	}
}
...
func TestValidate_ExemptedInternalPropertiesStillAccepted(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "obj1", "properties": {
...
func TestValidate_PropertyKeyShape(t *testing.T) {
	t.Run("refused", func(t *testing.T) {
		for _, doc := range []string{
			`{"formatVersion": "2.0", "properties": {"": "empty"}}`,
			`{"formatVersion": "2.0", "properties": {"a\nb": "newline"}}`,
			"{\"formatVersion\": \"2.0\", \"properties\": {\"a\\u0000b\": \"nul\"}}",
			"{\"formatVersion\": \"2.0\", \"properties\": {\"a\\u007fb\": \"del\"}}",
		} {
			assert.Error(t, Validate([]byte(doc), Options{}), doc)
		}
	})
	t.Run("accepted", func(t *testing.T) {
		// the shapes real keys have: bundled lowerCamel, a bson-hex custom key,
		// and the bare names old accounts carry. The pattern is
...
// the canonical example. The fixtures spell the CANONICAL snake_case slug
...
func TestValidate_PropertyValueShapeWarns(t *testing.T) {
	warningsFor := func(t *testing.T, doc string) []Issue {
		var got []Issue
		require.NoError(t, Validate([]byte(doc), Options{OnWarning: func(i Issue) { got = append(got, i) }}), doc)
		return got
	}

	t.Run("a date that is not a date", func(t *testing.T) {
		got := warningsFor(t, `{"formatVersion": "2.0", "id": "o1", "properties": {"due_date": "next Friday"}}`)
		require.Len(t, got, 1)
	...
	t.Run("the stored spelling is an address too", func(t *testing.T) {
		...
		got := warningsFor(t, `{"formatVersion": "2.0", "id": "o1", "properties": {"dueDate": "next Friday"}}`)
		require.Len(t, got, 1)
	...
	t.Run("a spelling the legend binds is checked as what it resolves to", func(t *testing.T) {
		got := warningsFor(t, `{"formatVersion": "2.0", "id": "o1",
	...
	t.Run("a checkbox that is not a boolean", func(t *testing.T) {
		got := warningsFor(t, `{"formatVersion": "2.0", "id": "o1", "properties": {"done": "yes"}}`)
		require.Len(t, got, 1)
	...
	t.Run("shapes the format does hold are quiet", func(t *testing.T) {
		...
		assert.Empty(t, warningsFor(t, `{"formatVersion": "2.0", "id": "o1", "properties": {
...
func TestInline_MentionAttributeIsSnakeCase(t *testing.T) {
	md := `ping <mention object_id="bafyid">Alice</mention>`
	text, marks, err := parseInline(md)
	require.NoError(t, err)
	assert.Equal(t, "ping Alice", text)
	...
	_, _, err = parseInline(`ping <mention objectId="bafyid">Alice</mention>`)
	require.Error(t, err)
...
func TestValidate_EnvelopeKeyAcceptsRealStoredKeys(t *testing.T) {
	for _, key := range []string{
		...
		"69aab06861fab2bc0d9afbe2_Ada Lovelace",
		"69a56205ccba0a47d8d8eb71_тогглы",
		"69bbfc78877a91b1d12d1a7c_JavaScript/TypeScript",
	} {
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "kind": "property_option", "id": "o1", "internal_key": %q}`, key)
		assert.NoError(t, Validate([]byte(doc), Options{}), "stored key %q must round-trip", key)
	}
}
...
func TestValidate_EnvelopeKeyRejectsUnreadable(t *testing.T) {
	for _, doc := range []string{
		`{"formatVersion": "2.0", "kind": "object_type", "id": "t1", "internal_key": ""}`,
		"{\"formatVersion\": \"2.0\", \"kind\": \"object_type\", \"id\": \"t1\", \"key\": \"a\\u0000b\"}",
		"{\"formatVersion\": \"2.0\", \"kind\": \"object_type\", \"id\": \"t1\", \"key\": \"a\\nb\"}",
	} {
		assert.Error(t, Validate([]byte(doc), Options{}), doc)
	}
}

// ---- Admission keyed off the raw document spelling ----
//
...
func TestValidate_DeniedKeysRefusedInCanonicalSpelling(t *testing.T) {
	...
	for key := range strippedDetailKeys() {
		...
		covered++
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "id": "obj1", "properties": {%q: "x"}}`, slug)
		err := Validate([]byte(doc), Options{})
...
func TestValidate_LegendCannotRebindOntoInternalKeys(t *testing.T) {
	for name, doc := range map[string]string{
		"resolution vector": `{"formatVersion": "2.0", "id": "o1", "property_internal_keys": {"prio": "uniqueKey"}, "properties": {"prio": "ot-page"}}`,
		"envelope id":       `{"formatVersion": "2.0", "id": "o1", "property_internal_keys": {"myid": "id"}, "properties": {"myid": "boom"}}`,
		"stripped key":      `{"formatVersion": "2.0", "id": "o1", "property_internal_keys": {"s": "spaceId"}, "properties": {"s": "other"}}`,
	} {
	...
	ok := `{"formatVersion": "2.0", "id": "o1", "property_internal_keys": {"prio": "6a32d4856761631534b22f85"}, "properties": {"prio": "high"}}`
	require.NoError(t, Validate([]byte(ok), Options{}))
...
func TestValidate_LayoutNameCheckedInCanonicalSpelling(t *testing.T) {
	err := Validate([]byte(`{"formatVersion": "2.0", "id": "o1", "properties": {"resolved_layout": "nonsense"}}`), Options{})
	require.Error(t, err)
	...
	doc := `{"formatVersion": "2.0", "id": "o1", "properties": {"resolved_layout": "todo"}}`
	require.NoError(t, Validate([]byte(doc), Options{}))
	_, snap, err := Unmarshal([]byte(doc), Options{GenerateId: seqIds("g")})
	require.NoError(t, err)
...
func TestValidate_LegendValueMustBeAWritableKey(t *testing.T) {
	t.Run("refused", func(t *testing.T) {
		for name, doc := range map[string]string{
			"empty":        `{"formatVersion": "2.0", "property_internal_keys": {"p": ""}}`,
			"over-long":    fmt.Sprintf(`{"formatVersion": "2.0", "property_internal_keys": {"p": %q}}`, strings.Repeat("k", maxPropertyKeyLen+1)),
			"control char": `{"formatVersion": "2.0", "property_internal_keys": {"p": "a\nb"}}`,
		} {
			assert.Error(t, Validate([]byte(doc), Options{}), name)
		}
	})
	t.Run("accepted", func(t *testing.T) {
		// the shapes real stored keys have: bson-hex, and option keys carrying
		// the option's own name, spaces and non-ASCII included
		doc := `{"formatVersion": "2.0", "property_internal_keys": {
...
func TestImport_AdmissionRunsOnTheResolvedKey(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "o1", "properties": {"prio": "ot-page"}}`
	require.NoError(t, Validate([]byte(doc), Options{}),
...
func TestExport_UnwritableSlugFallsBackToTheStoredKey(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "Marshal must never emit what its own Validate rejects (§11):\n%s", data)
...
func TestValidate_RecommendedListConflictCheckedInCanonicalSpelling(t *testing.T) {
	for _, spelling := range []string{
		...
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "kind": "object_type", "id": "t1", "internal_key": "page",
