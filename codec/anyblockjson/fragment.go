...
func validateFragmentRun(run []json.RawMessage, opts Options) ([]*jsonBlock, error) {
	...
	if _, err := validatePartialToDoc(payload, opts.NormalizeIndent, opts.OnWarning); err != nil {
		return nil, err
	}
	...
	for i, raw := range run {
		var jb jsonBlock
		if err := jsonUnmarshalUseNumber(raw, &jb); err != nil {
...
func UnmarshalBlocks(run []json.RawMessage, opts Options) (blocks []*model.Block, topIds []string, err error) {
	...
	doc := opts.fragmentDoc()
	doc.Blocks = jbs
	imp := &importer{opts: opts, doc: doc}
	// Reserve every authored id in the complete run before the first
	// generated block/table id. A later authored id is just as taken as an
	// earlier one; generation order must not decide identity.
	...
	if err := imp.finalize("/blocks"); err != nil {
		return nil, nil, err
	}
...
func UnmarshalBlock(raw json.RawMessage, forcedId string, opts Options) ([]*model.Block, error) {
	...
	doc.Blocks = jbs
	imp := &importer{opts: opts, doc: doc}
	if err := imp.claimReplacementId(jbs[0], forcedId); err != nil {
		return nil, err
	}
...
func MarshalBlockSubtree(subtree []*model.Block, opts Options) (json.RawMessage, error) {
	...
	if err := e.appendBlocksFlat(&out, []string{subtree[0].Id}, 0, false); err != nil {
		// Preserve the established whole/fragment error parity for a table
		// bound that the reached renderer itself reported. This check is
		// deliberately after rendering has failed: running the plan before the
		// renderer is what let an unreachable oversized descendant mask an
		// earlier parent error. Comparing the two diagnostics prevents a later
		// hidden table from taking precedence here as well.
		if gridErr := e.checkTableGridBounds(); gridErr != nil && gridErr.Error() == err.Error() {
			return nil, err
		}
	...
	// called only from buildDoc and buildTypeProperties, and neither
	...
	env.set("blocks", out)
	if err := validateCanonicalValueNumbers(env); err != nil {
		return nil, err
	}
