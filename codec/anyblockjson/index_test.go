...
func TestIndex_Roundtrip(t *testing.T) {
	...
	idx, err := UnmarshalIndex([]byte(doc), Options{})
...
func TestIndex_EntryPoint(t *testing.T) {
	t.Run("the declared entrypoint wins over widget order", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0",
			...
			"widgets": [{"target": "type-task", "layout": "view"}]}`), Options{})
	...
	t.Run("nothing declared means no entry point", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "name": "X"}`), Options{})
		require.NoError(t, err)
	...
	t.Run("falls back to the first widget naming an object", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0",
			"widgets": [{"target": "_recent"}, {"target": "page-home"}]}`), Options{})
	...
	t.Run("a reserved listing cannot be an entrypoint", func(t *testing.T) {
		for _, bad := range []string{"_widgets", "_graph", "_favorite", "_recent", "_all_objects"} {
			_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "entrypoint": "`+bad+`"}`), Options{})
	...
	t.Run("no entrypoint may enter the platform namespace", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "entrypoint": "_otpage"}`), Options{})
		require.Error(t, err)
...
func TestIndex_SpaceHomepage(t *testing.T) {
	...
	t.Run("an explicit value wins", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0",
			"entrypoint": "page-welcome", "homepage": "page-dashboard"}`), Options{})
	...
	t.Run("a reserved homepage is still allowed, deliberately", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0",
			"entrypoint": "page-home", "homepage": "_graph"}`), Options{})
	...
	t.Run("the wire spelling of a reserved screen is an ordinary id", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "homepage": "graph"}`), Options{})
		require.NoError(t, err)
...
func TestIndex_Validation(t *testing.T) {
	for _, tc := range []struct{ name, doc, want string }{
		{"version required", `{"name": "X"}`, "formatVersion"},
		{"unknown layout", `{"formatVersion": "2.0", "widgets": [{"target": "a", "layout": "grid"}]}`, "layout"},
		{"widget needs a target", `{"formatVersion": "2.0", "widgets": [{"layout": "link"}]}`, "target"},
		{"unknown property", `{"formatVersion": "2.0", "startingPage": "a"}`, "startingPage"},
		{"limit must be an integer", `{"formatVersion": "2.0", "widgets": [{"target": "a", "limit": 1.5}]}`, "limit"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, err := UnmarshalIndex([]byte(tc.doc), Options{})
			require.Error(t, err)
	...
	t.Run("reserved homepage names are accepted", func(t *testing.T) {
		for _, h := range []string{"_widgets", "_graph"} {
			_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "homepage": "`+h+`"}`), Options{})
	...
	t.Run("the bare listing name reserves nothing", func(t *testing.T) {
		for _, bare := range []string{"favorite", "recent", "set", "collection"} {
			...
			_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "entrypoint": "`+bare+`"}`), Options{})
	...
	t.Run("an unknown reserved listing is refused, and named", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "widgets": [
			{"target": "page-home"}, {"target": "_favourite"}]}`), Options{})
	...
	t.Run("an unknown reserved homepage is refused", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "homepage": "_last_opened"}`), Options{})
		require.Error(t, err)
	...
	t.Run("a newer version is rejected, naming both versions", func(t *testing.T) {
		...
		data := []byte(`{"formatVersion": "2.1", "name": "Wiki", "futureKey": true}`)
	...
	t.Run("a missing version is rejected", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"name": "Wiki"}`), Options{})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "formatVersion is required")
	})

	t.Run("a non-object index is rejected cleanly", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`[1, 2, 3]`), Options{})
		require.Error(t, err)
...
func TestIndex_Icon(t *testing.T) {
	idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "name": "Wiki",
		"icon": {"format": "file", "file": "acme-logo"}}`), Options{})
	...
	t.Run("an emoji index icon has no image id", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "icon": {"format": "emoji", "emoji": "📚"}}`), Options{})
		require.NoError(t, err)
	...
	t.Run("an emoji and an image are no longer both writable", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0",
			"icon": {"format": "emoji", "emoji": "📚", "file": "logo"}}`), Options{})
	...
	t.Run("the discriminator names the alternatives when it is missing", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "icon": {"emoji": "📚"}}`), Options{})
		require.Error(t, err)
	...
	t.Run("a letter avatar is a colour and nothing else", func(t *testing.T) {
		idx, err := UnmarshalIndex([]byte(`{"formatVersion": "2.0", "icon": {"format": "color", "color": "red"}}`), Options{})
		require.NoError(t, err)
