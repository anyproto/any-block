...
func (imp *importer) dataviewFromJSON(jb *jsonBlock) (*model.BlockContentDataview, error) {
	...
	// A view id is local to one dataview block. Census every authored id in
	// this dataview before generating any omissions, then reserve generated
	// ids in the same local set. This makes generation independent of view
	// order without incorrectly forbidding the same view id in two dataviews.
	viewIds := make(map[string]struct{}, len(jb.Views))
	for _, jv := range jb.Views {
		if jv.Id != "" {
			viewIds[jv.Id] = struct{}{}
		}
	}
	for _, jv := range jb.Views {
		viewId := jv.Id
		if viewId == "" {
			viewId = imp.genUniqueId(func(id string) bool {
				_, taken := viewIds[id]
				return taken
			})
		}
