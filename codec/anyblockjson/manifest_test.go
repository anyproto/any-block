...
func TestIndex_ManifestRefusals(t *testing.T) {
	t.Run("an undeclared manifest member is refused", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion":"2.0","manifest":{"templates":{}}}`), Options{})
		require.Error(t, err)
	})
	...
	t.Run("an empty path is refused", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion":"2.0","manifest":{"properties":""}}`), Options{})
		require.Error(t, err)
	})
}
...
func TestIndex_ManifestDoesNotLocateOptions(t *testing.T) {
	...
	t.Run("a bundle that still carries it is refused, not silently misread", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion":"2.0","manifest":{
			"options":{"bafyopt1":"relationsOptions/bafyopt1.anyblock.json"}}}`), Options{})
...
func TestIndex_ManifestBindsFileBlobs(t *testing.T) {
	...
	t.Run("a non-string or empty blob path is refused", func(t *testing.T) {
		_, err := UnmarshalIndex([]byte(`{"formatVersion":"2.0","manifest":{"files":{"bafyx":42}}}`), Options{})
		require.Error(t, err)
		_, err = UnmarshalIndex([]byte(`{"formatVersion":"2.0","manifest":{"files":{"bafyx":""}}}`), Options{})
		require.Error(t, err)
	})
}
