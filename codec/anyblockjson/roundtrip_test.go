...
func richSnapshot() *model.SmartBlockSnapshotBase {
	...
	blocks := []*model.Block{
		...
		textBlock("b2", model.BlockContentText_Paragraph, "Ship the new export with Alice",
			mark(mBold, 9, 19, ""), mark(mMention, 25, 30, "bafyreialice")),
...
func TestRoundTrip_State(t *testing.T) {
	...
	want := []*model.BlockContentTextMark{
		mark(mBold, 9, 19, ""),
		mark(mMention, 25, 30, "bafyreialice"),
	}
	...
	assert.Equal(t, strList("bafyreialice"), snap.Details.Fields["assignee"])
...
func TestCompactBlockLabels(t *testing.T) {
	opts := testOptions()
	...
	assert.Contains(t, s, `<mention object_id=\"bafyreialice\">`)
	assert.Contains(t, s, `"bafyreialice"`, "the assignee property value too")
	...
	assert.NotContains(t, s, `"alice": "bafyreialice"`)
	...
	assert.Equal(t, "bafyreialice", mention)
}
...
func TestImport_TitleAbsorption(t *testing.T) {
	...
	doc2 := `{"formatVersion": "2.0", "properties": {"name": "Kept"}, "blocks": [
...
func TestExplicitIndentZero(t *testing.T) {
	doc := `{"formatVersion": "2.0", "blocks": [{"indent": 0, "id": "a", "type": "paragraph", "text": "x"}]}`
...
func TestGeneratedDocs_ByteStable(t *testing.T) {
	...
	for i := 0; i < 300; i++ {
		...
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "properties": {"name": "Doc %d"}, "blocks": [%s]}`,
			i, strings.Join(blocks, ","))
		require.NoError(t, Validate([]byte(doc), Options{}), "case %d: generated doc must be valid: %s", i, doc)
		...
		require.NoError(t, Validate(canonical, Options{}), "case %d: canonical must validate: %s", i, canonical)
