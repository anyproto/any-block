...
func TestTypePropertiesImport(t *testing.T) {
  ...
  "type_settings": {"property_definitions": [
	...
	t.Run("empty array rebuilds all four lists as empty", func(t *testing.T) {
		// given
		doc := `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "task", "type_settings": {"property_definitions": []}}`
	...
	t.Run("absent type_properties leaves the lists absent", func(t *testing.T) {
		// given
		doc := `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "task"}`
...
func TestTypePropertiesValidation(t *testing.T) {
	t.Run("rejected outside type documents", func(t *testing.T) {
		// given
		doc := `{"formatVersion": "2.0", "type_settings": {"property_definitions": [{"property": "due_date"}]}}`
  ...
  "type_settings": {"property_definitions": [{"property": "due_date"}]}
	...
	t.Run("unknown section and no identity at all rejected by schema", func(t *testing.T) {
		for _, doc := range []string{
			`{"formatVersion": "2.0", "kind": "object_type", "type_settings": {"property_definitions": [{"property": "a", "section": "sidebar"}]}}`,
			`{"formatVersion": "2.0", "kind": "object_type", "type_settings": {"property_definitions": [{"format": "number"}]}}`,
			`{"formatVersion": "2.0", "kind": "object_type", "type_settings": {"property_definitions": [{"property": "a", "format": "status"}]}}`,
		} {
			assert.Error(t, Validate([]byte(doc), Options{}), strings.ReplaceAll(doc, "\n", " "))
		}
	})
	...
	t.Run("a name alone declares a property", func(t *testing.T) {
		require.NoError(t, Validate([]byte(
			`{"formatVersion": "2.0", "kind": "object_type", "internal_key": "recipe", "type_settings": {`+
				`"property_definitions": [{"name": "Cooking Time", "format": "number"}]}}`), Options{}))
	})
