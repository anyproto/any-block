...
func TestSpecClaim_NoSeparatorSurvivesRealNames(t *testing.T) {
	assert.Equal(t, "c#", vocabulary.ApiSlug("C#"),
		"a property named C# slugs with the separator INSIDE it")
	assert.Equal(t, "#1_priority", vocabulary.ApiSlug("#1 priority"),
		"and one can begin with it")
	assert.Equal(t, "c/c++", vocabulary.ApiSlug("C/C++"),
...
func TestSpecClaim_TheSigilIsNotDistinguishableFromData(t *testing.T) {
	...
	// it marks nothing
	assert.Equal(t, "@home", vocabulary.ApiSlug("@home"))
	...
	var _ func(data []byte, opts Options) error = Validate
...
func TestSpecClaim_TheStoreStepIsNeitherRemovableNorPromotable(t *testing.T) {
	...
	assert.Empty(t, vocabulary.RelationKeysByApiFold("Severity"),
	...
	// worth having at all
	assert.Equal(t, []domain.RelationKey{vocabulary.RelationKeyDueDate}, vocabulary.RelationKeysByApiFold("due-date"))
	...
	assert.Equal(t, []domain.TypeKey{vocabulary.TypeKeyTask}, vocabulary.TypeKeysByApiFold("Task"))
}
