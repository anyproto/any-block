...
// to a reader. Canonical output never derives a wire identifier. The
// authoring-bundle planner accepts the API-derived spelling only as a legacy
// input alias declared from that bundle's type name, then writes the NFC
// display name back; outside that compatibility door, the API slug stays the
// API surface's affair.
//
...
//     Space-backed vocabularies have honoured this from the start and the
//     interface never said so. The accept half is one guard: a term that is
//     some live entity's stored key returns ("", false) before the name
//     tables are consulted at all. The emit half is two: every label is
//     vetted against the stored-key set as it is granted, and the bundled
//     arm of `PropertySlug` re-checks the same set.
...
// to one holder), and the emit side yields to live stored keys — so this is
// above all a rule for hand-written implementations, which Options.Keys
// accepts from anyone.
...
// which is what a UI delete does — the deleted entity vacates its stored key
// while objects still carry it. A fully conforming vocabulary then binds a
// spelling that objects still carry as a stored key, so export writes the
// identity entry for it —
...
// vocabulary can apply it to whatever its source of truth stores; a
// space-backed vocabulary is expected to call them, and an implementation
// that answers with a raw stored key is still correct, merely less readable.
...
//
// A space-backed vocabulary implements it; the bundled-only default does not
// (bundled names are unique by CI guard, so neither question arises offline).
//
...
func (BundledKeyVocabulary) TypeSlug(key string) string {
	if vocabulary.HasObjectTypeByKey(domain.TypeKey(key)) {
