...
// one function that builds the object back — shared by Heart's extraction-time converter
...
// from it: the importer replaces it with the importing space's own derived
// widgets id, and the root block is renamed to match before the state is
// built. It only has to be stable and impossible for a bundle object to
// claim — `widgets` is a reserved bundle id already (IsReservedBundleId, via
// the homepage table).
...
var widgetObjectResidualKeys = map[string]bool{
	vocabulary.RelationKeyCreatedDate.String():      true,
	vocabulary.RelationKeyLastModifiedDate.String(): true,
}
...
func WidgetObjectResidualKey(key string, v *types.Value) bool {
	...
	return key == vocabulary.RelationKeyName.String() && v.GetStringValue() == ""
}
...
func widgetObjectConstantDetail(key string, v *types.Value) bool {
	switch key {
	case vocabulary.RelationKeyIsHidden.String():
		return v.GetBoolValue()
	case vocabulary.RelationKeyLayout.String(), vocabulary.RelationKeyResolvedLayout.String():
...
func widgetPair(wrapper *model.Block, byId map[string]*model.Block) (w Widget, linkId string, ok bool) {
	...
	for _, key := range lc.Relations {
		// A lifted key has no per-document legend in index.json, so mere
		// writability is not enough: its canonical wire spelling must read
		// back to this exact stored identity. For example, stored `dueDate`
		// writes as "Due date" and reads back to `dueDate`, while custom
		// `Due date` and `due_date` would silently rebind to that bundled
		// property. Keep the whole widget document for those non-fixed points.
		if !indexWidgetPropertyFixedPoint(key) {
			return w, "", false
		}
	}
...
// indexWidgetPropertyFixedPoint is the exact identity contract between the
// stored link relation and the index's write/read pair. Keeping it beside the
// lift makes it impossible to omit a widget property that MarshalIndex and
// UnmarshalIndex would silently bind to another property.
func indexWidgetPropertyFixedPoint(stored string) bool {
	_, _, fixed := indexWidgetPropertyBinding(stored)
	return fixed
}
...
// snapshot a converter puts in an archive, and the reconstruction a
// round-trip verifier holds against the original, one function so the two
// cannot drift. It returns nil when the index carries no sidebar state at
// all: a bundle declaring nothing gets no snapshot rather than an empty one.
//
// Four things about the block graph are load-bearing, none of them obvious,
// and each is a property the importing side relies on:
//
//   - The root block must carry smartblock content. The importer renames the
//     first block carrying it to the space's derived widgets id; with no such
//     block it *appends* a second root instead, the state's root becomes that
//     new block, and every wrapper is orphaned.
//   - Every wrapper must be reachable from the root. The merge walks the
//     state breadth-first from the root — a block the root does not reach is
//     simply not there.
//   - That traversal is also why root.ChildrenIds order is sidebar order:
//     the merge appends each widget to the existing widget object in
//     traversal order. So the order here is index.json's order.
//   - A wrapper gets exactly one child. The merge reads ChildrenIds[0] and
//     ignores the rest.
//
// The auto-widget ledger and switch ride along as details. On today's
// experience path they are inert — the importer merges only the BLOCKS into
// the space's own widget object — but they are the snapshot's truthful
// state, written so the path that starts reading details does not have to
// change this function. The §2c table records the inertness.
func WidgetsSnapshot(idx *Index) (*model.SmartBlockSnapshotBase, error) {
	...
	details := map[string]*types.Value{
		...
		vocabulary.RelationKeyLayout.String(): {Kind: &types.Value_NumberValue{
			NumberValue: float64(model.ObjectType_dashboard)}},
		vocabulary.RelationKeyResolvedLayout.String(): {Kind: &types.Value_NumberValue{
			NumberValue: float64(model.ObjectType_dashboard)}},
		// the widget object is never listed anywhere as an object
		vocabulary.RelationKeyIsHidden.String(): {Kind: &types.Value_BoolValue{BoolValue: true}},
	}
	...
	return &model.SmartBlockSnapshotBase{
		...
		ObjectTypes: []string{vocabulary.TypeKeyDashboard.URL()},
	}, nil
}
