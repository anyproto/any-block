...
// UnmarshalPropertyValueChecked is the lossless value-level import door. It
// applies the same recursive finite/round-trip-stable float64 policy as whole
// documents and block/query fragments, then performs the key's format-aware
// conversion. A rejected number is reported at a pointer relative to v.
//
// UnmarshalPropertyValue remains for source compatibility. On a numeric
// refusal it returns a nil *types.Value, which is detectably different from
// an authored JSON null (a non-nil Value_NullValue). New callers should use
// this checked form so they receive the diagnostic.
func UnmarshalPropertyValueChecked(key string, v any, opts Options) (*types.Value, error) {
	if isDroppedOnImport(key) {
		return nil, nil
	}
	if _, err := jsonToProtoValueChecked(v, ""); err != nil {
		return nil, err
	}
	imp := &importer{opts: opts, doc: opts.fragmentDoc()}
	value := imp.propertyValue(key, key, v)
...
// Unmarshal validates data and reconstructs a snapshot (§13). With Options{}
// or an equivalent non-widening bundled vocabulary, its acceptance agrees
// exactly with Validate. A wider/store-backed Options.Keys may add semantic
// admission refusals that byte-only validation cannot predict; those errors
// still wrap *ValidationError with the authored JSON path, and every Validate
// rejection reaches Unmarshal before vocabulary resolution.
...
func (imp *importer) claimAuthoredIds() {
	imp.claimAuthoredIdsExcept(nil)
}

// claimAuthoredIdsExcept builds the authored/table-owned census while
// omitting one block object's own id. UnmarshalBlock uses this for a forced
// replacement address: the payload id is discarded, so reserving it would
// make later generated ids depend on data that cannot enter the snapshot.
// Everything the block owns (table axes, the implicit grid, and cell
// descendants) is still claimed.
func (imp *importer) claimAuthoredIdsExcept(except *jsonBlock) {
...
// claimReplacementId makes forcedId the effective address of root. It
// replaces root.Id in the census rather than adding a second claim, rejects
// collisions with every authored/table-owned id below the root, and reserves
// the effective address before any wrapper or missing id can be generated.
func (imp *importer) claimReplacementId(root *jsonBlock, forcedId string) error {
	if forcedId == "" {
		imp.claimAuthoredIds()
		return nil
	}
	imp.claimAuthoredIdsExcept(root)
	if imp.idTaken(forcedId) {
		return &ValidationError{Issues: []Issue{{
			Path:    "/blocks/0/id",
			Message: fmt.Sprintf("forced replacement id %q collides with an authored or table-derived block id", forcedId),
		}}}
	}
	imp.claimId(forcedId)
	return nil
}
...
func (imp *importer) genId() string {
	return imp.claimId(imp.genUniqueId(imp.idTaken))
}

// genUniqueId calls the caller's generator exactly once and disambiguates
// that proposed base in the namespace supplied by taken. Block ids use the
// document-wide graph namespace through genId; dataview view ids use their
// own per-dataview namespace because view identity is local to its block.
func (imp *importer) genUniqueId(taken func(string) bool) string {
	...
	base := mint()
	if base == "" {
		// Empty is the authored-id sentinel and claimId deliberately does not
		// reserve it. Normalize one adversarial mint to a valid deterministic
		// base, then let the ordinary suffix/claim path do all collision work;
		// retrying an always-empty generator would never terminate.
		base = "b"
	}
...
// `/type`, `/template_for`, a property definition's `property` and both
// `object_types` slots refused that from the start; the ten slots that did
// not stored the empty key and then LOST the slot on the way back out — a
// column and a sort vanish, a property block and a link's shown-property list
// come back nameless, a filter re-exports as a node that filters on nothing.
//
...
// finalize completes the importer contract after a fragment or whole
// document has been built. Key-slot refusals are deferred by the deep block
// and dataview builders, so every public entry point must call this before it
// returns what those builders produced. fragmentPath remaps the document
// builder's coarse /blocks pointer onto the fragment surface that was read.
func (imp *importer) finalize(fragmentPath string) error {
	if imp.refusal != nil {
		issue := *imp.refusal
		if fragmentPath != "" && issue.Path == "/blocks" {
			issue.Path = fragmentPath
		}
...
func (imp *importer) warn(path, format string, args ...any) {
	imp.warnWithCode("", path, format, args...)
}

// warnWithCode is warn with a stable machine-readable semantic code. The
// code crosses the public OnWarning seam unchanged; path and prose remain
// free to improve without changing downstream safety decisions.
func (imp *importer) warnWithCode(code IssueCode, path, format string, args ...any) {
	if imp.opts.OnWarning == nil {
		return
	}
	imp.opts.OnWarning(Issue{Path: path, Message: fmt.Sprintf(format, args...), Code: code})
}
...
func (imp *importer) build() (model.SmartBlockType, *model.SmartBlockSnapshotBase, error) {
	...
	// answers for every pre-freeze document now, so at formatVersion 2.0 that
	...
	// caller (including Heart's extraction-time roundtrip, recovery, and
	// batch tools) — so the codec is the backstop.
	...
	// Deferred key-slot refusals and folded-participant warnings are part of
	// every importer entry point, not only the whole-document builder.
	if err := imp.finalize(""); err != nil {
		return 0, nil, err
	}
...
func (imp *importer) propertyValue(key, slug string, v any) *types.Value {
	// The legacy value-level API has no error channel. Refuse an unstable
	// number as nil before format-specific coercion can turn it into an empty
	// list or authored null. Whole and raw fragment doors have already emitted
	// a path-addressed validation error before reaching this point.
	if _, err := jsonToProtoValueChecked(v, ""); err != nil {
		return nil
	}
	if v == nil {
