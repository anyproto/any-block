...
// Two invariants hold the format together. Six violations of the first and four
// of the second have been found and fixed — every one of them in a place the
// rule had never been applied, though it was written down. Instances get
// their own regression tests in keyadmission_test.go; these are the
...
//	I2. Validate and Unmarshal agree exactly under Options{} or an equivalent
//	    non-widening bundled vocabulary (§12). A wider/store-backed Keys may
//	    narrow import acceptance only with a path-addressed semantic refusal;
//	    every Validate rejection still reaches Unmarshal.
//
...
// CompactBlockLabels, `r1-c1` is what a table derives for its own cell, `c_1-c1` is
...
func hostileSnapshot(n int) (model.SmartBlockType, *model.SmartBlockSnapshotBase) {
	...
	// Length and charset are deliberately unbounded here (§3): the app mints an
	// option key from the option's NAME, and a markdown/YAML import carries that
	// name verbatim, so a pasted paragraph or a pasted newline both reach this
	// slot. Both used to pass Marshal and fail the document's own Validate.
	storedKeys := []string{
		...
		"completion_status_" + strings.Repeat("x", 300),
		"completion_status_Not\nStarted", "completion_status_Not\tStarted",
	}
...
var (
	hostileSpaceId  = testfixtures.SpaceID
	...
	for name, variant := range variants {
		t.Run(name, func(t *testing.T) {
			for n := 0; n < 300; n++ {
				...
				var warned []Issue
				require.NoError(t, Validate(data, Options{OnWarning: func(i Issue) { warned = append(warned, i) }}),
					"seed %d produced:\n%s", n, data)
...
func TestInvariant_MarshalOutputValidates_RichFixture(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"compact": {Options{CompactBlockLabels: true, ResolveFormat: testFormatResolver, ResolveOptions: testResolver},
			legend("customStatus", map[string]string{"In progress": "opt1", "Done": "opt2"})},
...
var hostileDocs = []string{
	// The escape hatches: the schema left `root` an untyped object while the
	// reader decodes it into a struct, so a mistyped member validated and then
	// failed to decode — I2's only breach, and invisible until a case put a
	// non-string in the slot. `store` decodes to map[string]any and is here to
	// keep it that way.
	`{"formatVersion": "2.0", "id": "o1", "root": {"background_color": 1}}`,
	`{"formatVersion": "2.0", "id": "o1", "root": {"background_color": true}}`,
	`{"formatVersion": "2.0", "id": "o1", "root": {"background_color": []}}`,
	`{"formatVersion": "2.0", "id": "o1", "root": {"background_color": {"a": 1}}}`,
	`{"formatVersion": "2.0", "id": "o1", "root": {"fields": "not an object"}}`,
	`{"formatVersion": "2.0", "id": "o1", "root": {"unknown_member": 1}}`,
	`{"formatVersion": "2.0", "id": "o1", "store": {"anything": [1, {"a": null}]}}`,
	`{"formatVersion": "2.0"}`,
	`{"formatVersion": 2.0}`,
	`{"formatVersion": 2e0}`,
	`{"formatVersion": 2.5}`,
	`{"formatVersion": "2.1"}`,
	`{"formatVersion": "0.0"}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": 2048}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": 2048.0}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": 1e3}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": 1e30}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": -1}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "file", "size": 2048.5}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "widget", "limit": 10}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "widget", "limit": 1e1}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "widget", "limit": 1e20}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "widget", "limit": -3}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"id": "v1", "page_size": 50.0}]}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"id": "v1", "page_size": 1e19}]}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"id": "v1", "page_size": 0}]}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "table", "columns": [{"id": "c1", "width": 120.7}], "rows": []}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "table", "columns": [{"id": "c1", "width": 1e30}], "rows": []}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "table", "columns": [{"id": "c1", "width": -5}], "rows": []}]}`,
	`{"formatVersion": "2.0", "blocks": [{"indent": 0.0, "type": "paragraph", "text": "x"}]}`,
	`{"formatVersion": "2.0", "blocks": [{"indent": 1e1, "type": "paragraph", "text": "x"}]}`,
	`{"formatVersion": "2.0", "properties": {"name": "x", "size": 9007199254740993}}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "text": "<sub>x</sub>"}]}`,
	...
	`{"formatVersion": "2.0", "properties": {"num": 1e400}}`,
	`{"formatVersion": "2.0", "properties": {"num": 1e309}}`,
	`{"formatVersion": "2.0", "properties": {"num": 1e308}}`,
	`{"formatVersion": "2.0", "store": {"k": 1e400}}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "text": "x", "fields": {"w": 1e400}}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "table", "columns": [{"id": "c1", "width": 1e400}], "rows": []}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"id": "v1",
		"filters": [{"property": "p", "condition": "equal", "value": 1e400}]}]}]}`,
	`{"formatVersion": "2.0", "blocks": [{"type": "table", "columns": [{"id": "c1"}],
	...
	`{"formatVersion": "2.0", "properties": {"unique_key": "ot-page"}}`,
	`{"formatVersion": "2.0", "properties": {"space_id": "other"}}`,
	`{"formatVersion": "2.0", "properties": {"old_anytype_id": "legacy-1"}}`,
	`{"formatVersion": "2.0", "properties": {"source_file_path": "/x/y"}}`,
	`{"formatVersion": "2.0", "properties": {"resolved_layout": "nonsense"}}`,
	`{"formatVersion": "2.0", "properties": {"resolved_layout": "todo"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"prio": "uniqueKey"}, "properties": {"prio": "ot-page"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"myid": "id"}, "properties": {"myid": "boom"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"s": "spaceId"}, "properties": {"s": "other"}}`,
	// a benign rebind is the legend working as specified, and flows through
	`{"formatVersion": "2.0", "property_internal_keys": {"prio": "6a32d4856761631534b22f85"}, "properties": {"prio": "high"}}`,
	...
	`{"formatVersion": "2.0", "blocks": [{"type": "dataview",
	...
	`{"formatVersion": "2.0", "property_internal_keys": {"p": ""}}`,
	...
	`{"formatVersion": "2.0", "properties": {"a/b": "x"}}`,
	`{"formatVersion": "2.0", "properties": {"a~b": "x"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"a/b": ""}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"p": "a\nb"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"p": "` + strings.Repeat("k", 129) + `"}}`,
	...
	`{"formatVersion": "2.0", "properties": {"iconEmoji": "a", "icon_emoji": "b"}}`,
	`{"formatVersion": "2.0", "properties": {"dueDate": "2025-01-01T00:00:00Z", "due_date": "x"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"unique_key": "unique_key"}, "properties": {"unique_key": "custom"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"unique_key": "6a32d4856761631534b22f85"}, "properties": {"unique_key": "high"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"sneaky": "uniqueKey"}}`,
	`{"formatVersion": "2.0", "property_internal_keys": {"p": "oldAnytypeID"}}`,
	...
	`{"formatVersion": "2.0", "properties": {"prio": "bare"}}`,
	`{"formatVersion": "2.0", "properties": {"blank": "x"}}`,
	...
	`{"formatVersion": "2.0", "type": "task"}`,
	`{"formatVersion": "2.0", "type": "tsk"}`,
	`{"formatVersion": "2.0", "type": "blanktype"}`,
	`{"formatVersion": "2.0", "kind": "template", "type": "template", "template_for": "blanktype"}`,
	`{"formatVersion": "2.0", "type_internal_keys": {"task": "69bbfc78877a91b1d12d1a7c"}, "type": "task"}`,
	`{"formatVersion": "2.0", "type_internal_keys": {"object_type": "object_type"}, "type": "object_type"}`,
	`{"formatVersion": "2.0", "type_internal_keys": {"t": ""}}`,
	`{"formatVersion": "2.0", "type_internal_keys": {"t": "a\nb"}}`,
	...
	`{"formatVersion": "2.0", "option_ids": {"tag": {"High": "bafyreiopt"}}, "properties": {"tag": ["High"]}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"import issue": "bafyreiopt"}},
		"properties": {"tag": ["import issue"]},
		"blocks": [{"type": "link", "object_id": "miovm"}]}`,
	`{"formatVersion": "2.0", "option_ids": {"c#_lang": {"C#": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"#": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"has space": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"a\nb": "bafyreiopt"}}, "properties": {"tag": ["a\nb"]}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"": {"High": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": "bafyreiopt"}}`,
	`{"formatVersion": "2.0", "option_ids": {"ta\ng": {"High": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"` + strings.Repeat("p", maxPropertyKeyLen+1) + `": {"High": "bafyreiopt"}}}`,
	`{"formatVersion": "2.0", "option_ids": {"tag": {"` + strings.Repeat("n", maxPropertyKeyLen+1) + `": "bafyreiopt"}},
	...
	`{"formatVersion": "2.0", "kind": "template", "type": "template", "template_for": "task"}`,
	`{"formatVersion": "2.0", "kind": "template", "type": "tpl", "template_for": "task"}`,
	...
	`{"formatVersion": "2.0", "kind": "page", "type": "template"}`,
	// and the three invalid member combinations — Validate rejects
	// each, so I2 asks that Unmarshal reject them too rather than decoding a
	// page that meant to be a template
	`{"formatVersion": "2.0", "type": "template", "template_for": "task"}`,
	`{"formatVersion": "2.0", "kind": "page", "type": "template", "template_for": "task"}`,
	`{"formatVersion": "2.0", "kind": "template", "template_for": "task"}`,
}
		...
		t.Run(vocabName, func(t *testing.T) {
			for i, doc := range hostileDocs {
				t.Run(doc[:min(len(doc), 60)], func(t *testing.T) {
					valErr := Validate([]byte(doc), Options{})
...
func TestInvariant_ImportedDocumentReExportsValid(t *testing.T) {
	for _, doc := range hostileDocs {
		if Validate([]byte(doc), Options{}) != nil {
			continue
		}
		...
		require.NoError(t, err, doc)
		require.NoError(t, Validate(out, Options{}), "%s re-exported as:\n%s", doc, out)
...
func TestExport_ValidIdsAreNeverRenamed(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "obj1", "blocks": [
...
// I3: every domain identifier the format defines is snake_case (§1 Naming).
// The envelope's conventional `formatVersion` marker is the sole camelCase
// exception. The rule
...
func TestInvariant_VocabularyIsSnakeCase(t *testing.T) {
	...
	check := func(t *testing.T, where, ident string) {
		t.Helper()
		if strings.HasPrefix(ident, "$") || (where == "schema property" && ident == "formatVersion") {
			return
		}
