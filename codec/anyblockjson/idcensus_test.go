...
// TestExport_CompactBlockLabelsSurviveARoundTrip is guarantee 3 (§11) on the API's
...
func TestExport_CompactBlockLabelsSurviveARoundTrip(t *testing.T) {
	const (
...
func TestExport_NoCensusProbeWhenNoLocalIdIsWritten(t *testing.T) {
	snap := richSnapshot()
	...
	withIdsCompact := callsFor(Options{CompactBlockLabels: true})
	...
	require.NoError(t, err)
	b, err := Marshal(model.SmartBlockType_Page, snap, Options{OmitIds: true, CompactBlockLabels: true})
	require.NoError(t, err)
