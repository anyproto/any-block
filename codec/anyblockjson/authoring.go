...
var (
	authoringSchemaJSON           = formatschema.AuthoringObject()
	authoringIndexSchemaJSON      = formatschema.AuthoringIndex()
	authoringPropertiesSchemaJSON = formatschema.AuthoringProperties()
)
...
const (
	AuthoringSchemaURL           = schemaBaseURL + FormatVersion + "/authoring/object.schema.json"
	AuthoringIndexSchemaURL      = schemaBaseURL + FormatVersion + "/authoring/index.schema.json"
	AuthoringPropertiesSchemaURL = schemaBaseURL + FormatVersion + "/authoring/properties.schema.json"
)
...
func ValidateAuthoringIndex(data []byte) error {
	if _, err := UnmarshalIndex(data, Options{}); err != nil {
		return err
	}
...
func ValidateAuthoringPropertyDictionary(data []byte) error {
	if _, err := UnmarshalPropertyDictionary(data, Options{}); err != nil {
		return err
	}
...
func validateAuthoringSubset(data []byte, compile func() (*jsonschema.Schema, error), preamble string) error {
	...
	if !ok {
		return &ValidationError{Issues: []Issue{{Message: "document must be a JSON object"}}}
	}
	// The full reader immediately above has already accepted this version.
	// Repeat its in-memory normalization because the authoring schema is
	// intentionally compiled and applied as a separate second pass.
	if _, err := checkVersion(doc, nil); err != nil {
		return err
	}
	sch, err := compile()
	if err != nil {
...
func authoringSemantics(data []byte) error {
	...
	for _, term := range terms {
		...
		if key == vocabulary.RelationKeyName.String() {
			namesItself = true
		}
