...
func typeDoc(tp string) string {
	return `{"formatVersion": "2.0", "kind": "object_type", "id": "t1", "internal_key": "k",
	...
	for _, tc := range []struct{ key, want, bundled string }{
		...
		t.Run(tc.key, func(t *testing.T) {
			...
			require.NoError(t, Validate(
				[]byte(typeDoc(`{"property": "`+tc.key+`", "name": "`+tc.want+`"}`)), Options{OnWarning: func(i Issue) { got = append(got, i) }}))
			require.Len(t, got, 1)
