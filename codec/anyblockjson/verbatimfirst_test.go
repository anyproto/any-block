...
// entries. One family of silent corruptions traces back to the two halves
// disagreeing about this rule; these tests pin the settled behavior,
...
func TestValidate_LegendValueObeysTheDenyRule(t *testing.T) {
	for name, doc := range map[string]string{
		"resolution vector": `{"formatVersion": "2.0", "property_internal_keys": {"sneaky": "uniqueKey"}}`,
		"merge selector":    `{"formatVersion": "2.0", "property_internal_keys": {"p": "oldAnytypeID"}}`,
		"envelope key":      `{"formatVersion": "2.0", "property_internal_keys": {"myid": "id"}}`,
		"stripped key":      `{"formatVersion": "2.0", "property_internal_keys": {"s": "spaceId"}}`,
	} {
...
func TestImport_LegendBindingToACustomShadowKeyIsNotLaundering(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "o1", "property_internal_keys": {"x": "unique_key"}, "properties": {"x": "ot-page"}}`
	...
	require.NoError(t, err)
	require.NoError(t, Validate(out, Options{}), "re-export:\n%s", out)
}
...
func TestImport_SeamRefusesAnUnwritableResolvedKey(t *testing.T) {
	doc := `{"formatVersion": "2.0", "properties": {"blank": "x"}}`
	require.NoError(t, Validate([]byte(doc), Options{}),
...
func TestValidate_MirrorsTheSeamsDuplicateBindingRefusal(t *testing.T) {
	for name, doc := range map[string]string{
		"bundled twin":   `{"formatVersion": "2.0", "properties": {"pluralName": "a", "plural_name": "b"}}`,
		"date twin":      `{"formatVersion": "2.0", "properties": {"dueDate": "x", "due_date": "y"}}`,
		"legend-induced": `{"formatVersion": "2.0", "property_internal_keys": {"prio": "customKey"}, "properties": {"prio": 1, "customKey": 2}}`,
	} {
		t.Run(name, func(t *testing.T) {
			err := Validate([]byte(doc), Options{})
