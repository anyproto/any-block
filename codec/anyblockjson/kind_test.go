...
func TestImport_ChatKind(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "chat-wiki", "kind": "chat", "internal_key": "wikiChat",
...
func TestValidate_ChatDerivedNameRejected(t *testing.T) {
	_, _, err := Unmarshal([]byte(`{"formatVersion": "2.0", "kind": "chat_derived", "internal_key": "k"}`),
		Options{GenerateId: seqIds("g")})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "kind")

	sbType, _, err := Unmarshal([]byte(`{"formatVersion": "2.0", "kind": "chat_object", "internal_key": "k"}`),
		Options{GenerateId: seqIds("g")})
	require.NoError(t, err)
