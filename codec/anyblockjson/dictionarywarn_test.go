...
const dictHead = `"$schema":"https://schemas.anytype.io/anyblock/2.0/properties.schema.json","formatVersion":"2.0",`

func readDict(t *testing.T, doc string) (*PropertyDictionary, []Issue) {
	t.Helper()
	var warns []Issue
	d, err := UnmarshalPropertyDictionary([]byte(doc), Options{OnWarning: func(i Issue) { warns = append(warns, i) }})
...
func bundledRelationKeys() []string {
	urls := vocabulary.ListRelationsUrls()
