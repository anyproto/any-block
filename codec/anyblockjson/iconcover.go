...
// un-lift.
var (
	detailKeyIconEmoji  = vocabulary.RelationKeyIconEmoji.String()
	detailKeyIconImage  = vocabulary.RelationKeyIconImage.String()
	detailKeyIconName   = vocabulary.RelationKeyIconName.String()
	detailKeyIconOption = vocabulary.RelationKeyIconOption.String()
	detailKeyCoverId    = vocabulary.RelationKeyCoverId.String()
	detailKeyCoverType  = vocabulary.RelationKeyCoverType.String()
	detailKeyCoverScale = vocabulary.RelationKeyCoverScale.String()
	detailKeyCoverX     = vocabulary.RelationKeyCoverX.String()
	detailKeyCoverY     = vocabulary.RelationKeyCoverY.String()
)
...
// Icon is one of the two typed fields (§2b), Cover being the other. Both are
// exported because they are the shape a caller writing a document or a
// bundle index has to build, and because an API layer needs the same union
// and should adopt this one rather than mint a second.
//
...
// object shows above its title, in the shape a caller writing a document or
// a bundle index has to build.
//
// Exactly one variant's members are populated at a time and `Format` says
// which: `color` and `gradient` each name a palette entry, and `image` names
// an image object in the bundle, optionally with a `Source` saying where the
// image came from. The framing members (`Scale`, `X`, `Y`) belong to an
// image and to nothing else. Every member is typed — unlike Icon's `Color`
// there is no raw-number arm, because a cover colour is only ever a name.
type Cover struct {
	Format   string  `json:"format"`
	File     string  `json:"file"`
