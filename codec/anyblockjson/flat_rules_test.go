...
// Cells cannot contain tables (the schema's recursion cut) — a
...
// The schema's integer type admits integer-valued floats
...
func TestIndent_FloatForms(t *testing.T) {
	...
	t.Run("V2 fires on float indent under a leaf", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "blocks": [{"type": "divider"}, {"indent": 1.0, "type": "paragraph", "text": "x"}]}`
	...
	t.Run("V1 fires on float jump", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "text": "a"}, {"indent": 5.0, "type": "paragraph", "text": "b"}]}`
		valErr, _ := agree(t, doc)
		require.Error(t, valErr)
	...
	t.Run("valid float forms import with the right depth and canonicalize", func(t *testing.T) {
		for _, form := range []string{"1.0", "1e0"} {
			doc := fmt.Sprintf(`{"formatVersion": "2.0", "blocks": [
				{"id": "a", "type": "toggle", "text": "t"},
				{"indent": %s, "id": "b", "type": "paragraph", "text": "x"}
			]}`, form)
			require.NoError(t, Validate([]byte(doc), Options{}), form)
...
// A snapshot nested deeper than the F4 bound must fail Marshal
...
func TestMarshal_OnWarningDegradesOverDeep(t *testing.T) {
	...
	require.NoError(t, Validate(data, Options{}), "clamped indents stay within the format bound")
}

// Unknown properties are rejected with a message naming the key;
// `children` additionally gets the flat-migration hint.
func TestValidate_UnknownPropertyMessages(t *testing.T) {
	err := Validate([]byte(`{"formatVersion": "2.0", "blocks": [{"type": "toggle", "children": [{"type": "paragraph"}]}]}`), Options{})
	require.Error(t, err)
	...
	err = Validate([]byte(`{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "banana": 1}]}`), Options{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), `/blocks/0/banana: property "banana" is not allowed`)
}

// The F10 array-form cell round-trips byte-stable.
func TestRoundTrip_CellArrayForm(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, Validate(first, Options{}))
...
// Every V2 leaf type actually drops children on export, and the
...
// Lenient clamps land on the containment checks — a clamped
...
func TestNormalizeIndent_ContainmentStillErrors(t *testing.T) {
	t.Run("clamped under a leaf", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "blocks": [{"type": "divider"}, {"indent": 5, "type": "paragraph", "text": "x"}]}`
	...
	t.Run("clamped non-column under a row", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "blocks": [{"type": "row"}, {"indent": 7, "type": "paragraph", "text": "x"}]}`
		_, _, err := Unmarshal([]byte(doc), Options{GenerateId: seqIds("g"), NormalizeIndent: true})
		require.Error(t, err)
...
// The prefix property holds at real-world depth (≥ 6), not just
...
func TestValidate_PrefixProperty_Deep(t *testing.T) {
	...
	doc := `{"formatVersion": "2.0", "blocks": [` + strings.Join(parts, ",") + `]}`
	...
	for n := 0; n <= len(parsed.Blocks); n++ {
		...
		prefix := `{"formatVersion": "2.0", "blocks": [` + strings.Join(blockParts, ",") + `]}`
