...
func TestWidgetLimit_OneStatementOfTheCap(t *testing.T) {
	...
	t.Run("the two doors behave as the two statements say", func(t *testing.T) {
		...
		_, err := UnmarshalIndex([]byte(`{"formatVersion":"2.0","widgets":[{"target":"page-a","limit":100}]}`), Options{})
		require.NoError(t, err)
		_, err = UnmarshalIndex([]byte(`{"formatVersion":"2.0","widgets":[{"target":"page-a","limit":101}]}`), Options{})
		require.Error(t, err)
		...
		assert.NoError(t, Validate([]byte(
			`{"formatVersion":"2.0","id":"o1","blocks":[{"id":"b1","type":"widget","limit":101}]}`), Options{}))
	})
}
