...
// hardening_test.go — what hostile or degenerate input may not do: break a
// round trip (deep-link marks, brackets inside link destinations), recurse or
// run quadratically (cyclic children, the parse boundary), or mint an
// ambiguous id (shared and duplicate ids, compact-label suffix collisions).
// The §8 resource bounds that close the DoS half are pinned here too.
...
// A Link mark whose target is an object deep-link renders
...
// A ']' (or backtick) inside a link destination must be
...
// Cyclic ChildrenIds must not recurse into a fatal stack
...
// A block shared by two parents is emitted once; duplicate
...
func TestExport_SharedAndDuplicateIds(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "shared child must be emitted once")
	...
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "duplicate column ids must be dropped")
}

// The CompactBlockLabels path guards nil inner content. The companion concern — a
// refs key outside the schema charset — is closed by deletion: no object id
// is labelled at all now, so what stands in its place is the statement that
// an id no charset would have admitted travels verbatim.
func TestExport_CompactBlockLabelsHardening(t *testing.T) {
	...
	for _, c := range contents {
		...
		require.NotPanics(t, func() {
			_, _ = Marshal(model.SmartBlockType_Page, snap, Options{CompactBlockLabels: true})
		}, "content %T with CompactBlockLabels", c)
	}
...
// The parse boundary must stay effectively linear. Every input here
...
// The wiring dispatches on the version/$schema markers (§13).
func TestDetectFormat(t *testing.T) {
	v, schema, ok := DetectFormat([]byte(`{"$schema": "` + SchemaURL + `", "formatVersion": "2.0"}`))
	require.True(t, ok)
	assert.Equal(t, "2.0", v)
	assert.Equal(t, SchemaURL, schema)

	v, _, ok = DetectFormat([]byte(`{"formatVersion": "2.1"}`))
	require.True(t, ok)
	assert.Equal(t, "2.1", v)
	v, schema, ok = DetectFormat([]byte(`{"$schema": "legacy", "version": 2}`))
	require.True(t, ok)
	assert.Equal(t, "2.0", v)
	assert.Equal(t, "legacy", schema)

	_, _, ok = DetectFormat([]byte(`{"blocks": []}`))
	assert.False(t, ok)
	_, _, ok = DetectFormat([]byte(`{"formatVersion": 2.0}`))
	assert.False(t, ok)
	_, _, ok = DetectFormat([]byte(`{"formatVersion": "2.0.0"}`))
	assert.False(t, ok)
	_, _, ok = DetectFormat([]byte(`not json`))
	assert.False(t, ok)
}

// The default id generator (no GenerateId option) mints
// editor-shaped 24-hex ids.
func TestImport_DefaultIdGenerator(t *testing.T) {
	_, snap, err := Unmarshal([]byte(`{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "text": "x"}]}`), Options{})
	require.NoError(t, err)
...
func TestExport_SuffixCollisionFallsBackToFullId(t *testing.T) {
	...
	solo, err := Marshal(model.SmartBlockType_Page, snap, Options{CompactBlockLabels: true})
	require.NoError(t, err)
