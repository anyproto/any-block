...
// error. I1 (Marshal never emits what its own Validate rejects) and I2 are
// asserted on every arm: exact Validate/Unmarshal agreement for Options{} or
// an equivalent non-widening bundled vocabulary, and only path-addressed
// semantic narrowing under wider/store-backed Keys. The §11 fixpoint — a
// second export of the re-import is byte-identical — is asserted too.
...
func TestRawNames_EdgeWhitespaceIsCarriedAndWarned(t *testing.T) {
	...
	var warns []Issue
	require.NoError(t, Validate(data, Options{OnWarning: func(i Issue) { warns = append(warns, i) }}),
	...
	doc := `{"formatVersion":"2.0","id":"o1","property_internal_keys":{"Star️":"` + key + `"},` +
		`"properties":{"Star️":1}}`
	warns = nil
	require.NoError(t, Validate([]byte(doc), Options{OnWarning: func(i Issue) { warns = append(warns, i) }}))
	found := false
	for _, w := range warns {
...
func TestRawNames_TypeScopedResolution(t *testing.T) {
	...
	t.Run("the declared type singles the claimant out", func(t *testing.T) {
		doc := `{"formatVersion":"2.0","id":"o1","type":"Sprint","properties":{"Projects":"resolved"}}`
	...
	t.Run("a type that cannot place the name errors loudly", func(t *testing.T) {
		doc := `{"formatVersion":"2.0","id":"o1","type":"Page","properties":{"Projects":"?"}}`
	...
	t.Run("the legend outranks the whole question", func(t *testing.T) {
		doc := `{"formatVersion":"2.0","id":"o1","type":"Page",` +
...
func TestRawNames_VerbatimTermWarnings(t *testing.T) {
	...
	t.Run("a stale or guessed name mints a phantom, and says so", func(t *testing.T) {
		msgs := collect(`{"formatVersion":"2.0","id":"o1","properties":{"Budget":"1", "b1": {"x": 1}}}`,
			Options{Keys: vocab})
	...
	t.Run("the glued annotation names the live name it extends", func(t *testing.T) {
		msgs := collect(`{"formatVersion":"2.0","id":"o1","properties":{"Lists [in work] (text)":"x"}}`,
			Options{Keys: vocab})
		joined := strings.Join(msgs, "\n")
	...
	t.Run("a bundled name's glue is caught with no vocabulary at all", func(t *testing.T) {
		msgs := collect(`{"formatVersion":"2.0","id":"o1","properties":{"Creation date (text)":"x"}}`,
			Options{})
		joined := strings.Join(msgs, "\n")
	...
	t.Run("one term, one warning, however many slots name it", func(t *testing.T) {
		doc := `{"formatVersion":"2.0","id":"o1","properties":{"Budget":"1"},"blocks":[
...
func TestRawNames_AttributionNeverContestsASpelling(t *testing.T) {
	...
	snap := &model.SmartBlockSnapshotBase{Details: &types.Struct{Fields: map[string]*types.Value{
		"id":      pbtypes.String("o1"),
		"creator": pbtypes.String(testfixtures.ParticipantID("a.b", testfixtures.AlternateAccountIdentity)),
