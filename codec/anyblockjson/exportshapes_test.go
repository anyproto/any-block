...
// exportshapes_test.go — the value shapes the proto model allows and the
// schema does not: verbatim structs and nulls, an empty sort key, nil inner
// content, out-of-range enums, a non-list `objects` collection. Export must
// emit a document its own Validate accepts, or refuse; the inline mark and
// import cases here are the same rule at the other two seams.
...
// A same-param merge that extends an accepted range must re-run
...
// A bare link destination starting with '<' must not be misread
...
// Brackets/backticks inside tag attribute values must be
...
// Verbatim property passthrough (structs, nulls inside lists)
...
func TestExport_VerbatimPropertyShapes(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "canonical export must pass its own schema")
...
// Revisited under raw names: a Page whose type key is `template`
...
func TestExport_PageWithTemplateTypeKeepsKind(t *testing.T) {
	...
	t.Run("the display-name spelling derives its kind", func(t *testing.T) {
		...
		require.NoError(t, Validate(data, Options{}), "I1")
...
// A stray properties.id / properties.type in the document must not
...
// property-key admission rules landed it is refused by name,
// and the envelope stays the only place either one is set.
func TestImport_PropertiesIdDoesNotLeak(t *testing.T) {
	doc := `{"formatVersion": "2.0", "id": "realid", "properties": {"id": "fakeid", "type": "faketype", "name": "N"}}`
	...
	_, snap, err := Unmarshal([]byte(`{"formatVersion": "2.0", "id": "realid", "properties": {"name": "N"}}`),
...
// Tables nested inside table cells join the id-uniqueness domain
...
func TestValidate_NestedTableInCell(t *testing.T) {
	...
	err = Validate([]byte(dupIdInCellArray), Options{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "duplicate id")

	badInline := `{"formatVersion": "2.0", "blocks": [
		{"type": "table", "columns": [{"id": "c1"}], "rows": [{"id": "r1", "cells": [[
			{"type": "toggle", "text": "cell"},
			{"indent": 1, "type": "paragraph", "text": "<u>unclosed"}
		]]}]}
	]}`
	err = Validate([]byte(badInline), Options{})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "inline markup")
}

// A non-list "objects" value in the internal store stays in
...
// formatVersion is a public major.minor identifier, not a JSON number: using
// a string preserves the distinction between future 2.10 and 2.1 releases.
func TestValidate_FormatVersionIsString(t *testing.T) {
	require.NoError(t, Validate([]byte(`{"formatVersion": "2.0"}`), Options{}))
	require.Error(t, Validate([]byte(`{"formatVersion": 2.0}`), Options{}))
	require.Error(t, Validate([]byte(`{"formatVersion": 2.5}`), Options{}))
}
...
func TestExport_OutOfRangeEnums(t *testing.T) {
	...
	data, err := Marshal(model.SmartBlockType_Page, snap, Options{})
	require.NoError(t, err)
	require.NoError(t, Validate(data, Options{}), "out-of-range enums must not produce invalid output")
