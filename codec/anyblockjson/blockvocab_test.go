...
func TestStructuralTypesAreDroppedOnImport(t *testing.T) {
	...
	_, plain, err := Unmarshal([]byte(`{"formatVersion":"2.0","type":"page","blocks":[{"type":"paragraph","text":"body"}]}`), Options{})
	...
	for typ := range structuralBlockTypes {
		t.Run(typ, func(t *testing.T) {
			...
			doc := fmt.Sprintf(`{"formatVersion":"2.0","type":"page","blocks":[%s,{"type":"paragraph","text":"body"}]}`, structural)
