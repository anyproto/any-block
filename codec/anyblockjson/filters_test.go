...
func TestQueryFragments_FinalizeDeferredImporterState(t *testing.T) {
	t.Run("filter key refusal", func(t *testing.T) {
		_, err := UnmarshalFilters(json.RawMessage(
			`[{"property":"blank","condition":"equal","value":"x"}]`),
			Options{Keys: blankKeyVocab{}})

		var ve *ValidationError
		require.ErrorAs(t, err, &ve)
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/filters", ve.Issues[0].Path)
	})

	t.Run("sort key refusal", func(t *testing.T) {
		_, err := UnmarshalSorts(json.RawMessage(`[{"property":"blank"}]`),
			Options{Keys: blankKeyVocab{}})

		var ve *ValidationError
		require.ErrorAs(t, err, &ve)
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/sorts", ve.Issues[0].Path)
	})

	t.Run("folded participant warning", func(t *testing.T) {
		var warnings []Issue
		opts := Options{
			ResolveFormat: func(domain.RelationKey) (model.RelationFormat, bool) {
				return model.RelationFormat_object, true
			},
			OnWarning: func(issue Issue) { warnings = append(warnings, issue) },
		}
		_, err := UnmarshalFilters(json.RawMessage(
			`[{"property":"assignee","condition":"in","value":["`+foldIdentity+`"]}]`), opts)

		require.NoError(t, err)
		require.Len(t, warnings, 1)
		assert.Equal(t, "/filters", warnings[0].Path)
...
func TestUnmarshalFilters_FragmentAgreesWithDocument(t *testing.T) {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			doc := `{"formatVersion":"2.0","type":"page","blocks":[{"type":"dataview","views":[{"` +
				tc.member + `":` + tc.raw + `}]}]}`
			...
			docErr := Validate([]byte(doc), Options{})
