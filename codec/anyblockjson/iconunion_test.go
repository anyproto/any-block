...
func TestValidate_ARestrictedIconSlotNamesOneUnion(t *testing.T) {
	...
	for name, doc := range map[string]string{
		"a cover variant on a callout": `{"formatVersion": "2.0", "type": "page", "blocks": [
			{"type": "callout", "icon": {"format": "image", "file": "bafy1"}, "text": "x"}]}`,
		"an invented format on a callout": `{"formatVersion": "2.0", "type": "page", "blocks": [
			{"type": "callout", "icon": {"format": "url", "url": "http://x"}, "text": "x"}]}`,
		"an object-only variant on a callout": `{"formatVersion": "2.0", "type": "page", "blocks": [
	...
	t.Run("valid icons still validate", func(t *testing.T) {
		for _, doc := range []string{
			`{"formatVersion": "2.0", "type": "page", "icon": {"format": "emoji", "emoji": "📕"}}`,
			`{"formatVersion": "2.0", "type": "page", "icon": {"format": "icon", "name": "rocket"}}`,
			`{"formatVersion": "2.0", "type": "page", "blocks": [{"type": "callout", "icon": {"format": "emoji", "emoji": "📕"}, "text": "x"}]}`,
		} {
			require.NoError(t, Validate([]byte(doc), Options{}), fmt.Sprintf("doc: %s", doc))
		}
	})
}
