...
const tableDoc = `{"formatVersion": "2.0", "id": "p1", "blocks": [{"type": "table",
	"columns": [{}, {}],
	"rows": [{"cells": ["a", "b"]}]}]}`
...
func TestImport_SanitizedTableIdsStayUnique(t *testing.T) {
	...
	n := 0
	_, snap, err := Unmarshal([]byte(`{"formatVersion": "2.0", "id": "p1", "blocks": [{"type": "table",
...
func TestExport_DashedTableIdsAreNormalized(t *testing.T) {
	...
	assert.NoError(t, Validate(data, Options{}), "Marshal must not emit what Validate rejects")
...
func TestImport_GeneratedGridIsClaimed(t *testing.T) {
	tableOnly := `{"formatVersion": "2.0", "id": "p1", "blocks": [
	...
	t.Run("an authored block is already sitting on the grid", func(t *testing.T) {
		doc := fmt.Sprintf(`{"formatVersion": "2.0", "id": "p1", "blocks": [
			{"type": "paragraph", "id": %q, "text": "authored"},
			{"type": "table", "columns": [{}], "rows": [{"cells": ["cell"]}]}]}`, derived)
		require.NoError(t, Validate([]byte(doc), Options{}), "the document itself is legal: %s", doc)
...
func TestMarshalBlockSubtree_ReservesFromTheCallersRoot(t *testing.T) {
	...
	doc, err := json.Marshal(map[string]any{"formatVersion": FormatVersion, "blocks": env.Blocks})
	require.NoError(t, err)
	assert.NoError(t, Validate(doc, Options{}), "Marshal must not emit what Validate rejects:\n%s", fragment)
...
func TestImport_GeneratedTableInnerIdsKeepTheirName(t *testing.T) {
	doc := `{"formatVersion": "2.0", "blocks": [{"type": "table",
