...
func declaredAuthoringTypes(t *testing.T, documents map[string][]byte) *AuthoringTypeVocabulary {
	t.Helper()
	v, err := PlanAuthoringTypeVocabulary(documents, AuthoringVocabularyPlanOptions{})
...
func requireSubsetObject(t *testing.T, doc string) {
	...
	require.NoError(t, Validate(data, Options{}), "subset invariant: an authoring-valid document must be full-valid:\n%s", doc)
...
func TestAuthoringExample_HabitTracker(t *testing.T) {
	...
	objectData := make(map[string][]byte, len(objectFiles))
	for _, rel := range objectFiles {
		objectData[rel] = readFile(rel)
	}
	typeVocabulary := declaredAuthoringTypes(t, objectData)
	...
	for _, rel := range objectFiles {
		t.Run(rel, func(t *testing.T) {
			...
			var warnings []Issue
			require.NoError(t, Validate(data, Options{OnWarning: func(i Issue) { warnings = append(warnings, i) }}))
			assert.Empty(t, warnings, "the worked example must be warning-free")
			require.NoError(t, ValidateAuthoring(data))

			// the real codec
			kind, snapshot, err := Unmarshal(data, Options{Keys: typeVocabulary})
			require.NoError(t, err)
			...
			if doc.Kind != "object_type" {
				...
				storedKey, resolved := typeVocabulary.TypeKey(doc.Type)
				_, custom := typeVocabulary.byKey[storedKey]
				if resolved && custom {
					assert.Equal(t, []string{"ot-" + storedKey}, snapshot.GetObjectTypes(),
						"bundle import binds the declared display name to its stored key")
					canonical, err := Marshal(kind, snapshot, Options{Keys: typeVocabulary})
					require.NoError(t, err)
					assert.Equal(t, typeVocabulary.byKey[storedKey], decodeEnvelope(t, canonical).Type,
						"canonical re-export returns to the declared display name")
				}
			}
	...
	t.Run("properties.json", func(t *testing.T) {
		...
		var warnings []Issue
		dict, err := UnmarshalPropertyDictionary(data, Options{OnWarning: func(i Issue) { warnings = append(warnings, i) }})
		require.NoError(t, err)
		assert.Empty(t, warnings, "the worked example must be warning-free")
	...
	t.Run("the bundle is coherent", func(t *testing.T) {
		...
		for term, rel := range objectTypeTerms {
			...
			storedKey, resolved := typeVocabulary.TypeKey(term)
			_, declared := typeVocabulary.byKey[storedKey]
			declared = declared && resolved
			assert.True(t, declared,
				"%s has type display name %q, which no type document in the bundle declares", rel, term)
		}
	})
}

type authoringPropertyResolver struct {
	byID map[string]PropertyDefinition
}

func (r *authoringPropertyResolver) PropertyId(def PropertyDefinition) (string, bool) {
	if r.byID == nil {
		r.byID = map[string]PropertyDefinition{}
	}
	id := "authoring-" + string(def.Key)
	r.byID[id] = def
	return id, true
}

func (r *authoringPropertyResolver) PropertyById(id string) (PropertyDefinition, bool) {
	def, ok := r.byID[id]
	return def, ok
}

// A bundle-local custom type is addressed like every other type: its NFC
// display name is the document term and its explicit internal_key is the
// stored identity behind that term. This exercises all three type-key slots
// an author writes and both canonicalization directions. The legacy stored
// key remains accepted by verbatim fallback, but it is never written back.
func TestAuthoringCustomTypeReferencesUseDeclaredDisplayName(t *testing.T) {
	const (
		displayName = "Café Ritual"
	...
	for name, data := range map[string][]byte{
		"type declaration and object_types": typeDocument,
	...
	typeKind, typeSnapshot, err := Unmarshal(typeDocument, opts)
	require.NoError(t, err)
	require.Len(t, resolver.byID, 1)
	for _, def := range resolver.byID {
		assert.Equal(t, []string{storedKey}, def.ObjectTypes,
			"object_types display name must bind to the custom stored key")
	}
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"ot-" + storedKey}, ordinarySnapshot.GetObjectTypes())
	ordinaryCanonical, err := Marshal(ordinaryKind, ordinarySnapshot, Options{Keys: vocabulary})
	require.NoError(t, err)
	assert.Equal(t, displayName, decodeEnvelope(t, ordinaryCanonical).Type)

	templateKind, templateSnapshot, err := Unmarshal(template, opts)
	require.NoError(t, err)
	...
	require.NoError(t, err, "an exact stored key remains input-compatible")
	legacyCanonical, err := Marshal(legacyKind, legacySnapshot, Options{Keys: vocabulary})
	require.NoError(t, err)
	assert.Equal(t, displayName, decodeEnvelope(t, legacyCanonical).Type,
		"legacy stored-key input canonicalizes to the declared display name")
	assert.NotContains(t, string(legacyCanonical), `"type": "habit_record_v2"`)
}
...
func TestAuthoringSubset_StructuralFixtures(t *testing.T) {
	fixtures := map[string]string{
		"minimal document": `{"formatVersion": "2.0"}`,
		"a full page envelope": `{"formatVersion": "2.0", "id": "page-a", "type": "page",
		...
		"nested blocks": `{"formatVersion": "2.0", "blocks": [
		...
		"columns": `{"formatVersion": "2.0", "blocks": [
			{"type": "row"},
			{"indent": 1, "type": "column"},
			{"indent": 2, "type": "paragraph", "text": "left"},
			{"indent": 1, "type": "column"},
			{"indent": 2, "type": "paragraph", "text": "right"}]}`,
		"a table with empty and padded cells": `{"formatVersion": "2.0", "blocks": [
			{"type": "table",
			 "columns": [{}, {}, {}],
			 "rows": [
		...
		"filters, groups and sorts": `{"formatVersion": "2.0", "blocks": [
			{"type": "dataview",
		...
		"inline markup": `{"formatVersion": "2.0", "blocks": [
		...
		"embeds and bookmarks": `{"formatVersion": "2.0", "blocks": [
...
func TestAuthoringSubset_EveryObjectEnumValueIsFullValid(t *testing.T) {
	...
	typeDoc := func(settings string) string {
		return `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t1",
			"properties": {"name": "T"}, "type_settings": {` + settings + `}}`
	}
	dataviewDoc := func(view string) string {
		return `{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{` + view + `}]}]}`
	}
	...
	sweep("kind", schemaEnum(t, schema, "properties", "kind", "enum"), func(v string) string {
		...
		case "template":
			return `{"formatVersion": "2.0", "kind": "template", "type": "template", "template_for": "T"}`
		default:
			return `{"formatVersion": "2.0", "kind": "` + v + `"}`
		}
	})
	...
	sweep("block type", schemaEnum(t, schema, "$defs", "block", "properties", "type", "enum"), func(v string) string {
		...
		return `{"formatVersion": "2.0", "blocks": [` + b + `]}`
	})
	...
	sweep("layout_align", schemaEnum(t, schema, "$defs", "blockAlign", "enum"), func(v string) string {
		return `{"formatVersion": "2.0", "properties": {"layout_align": "` + v + `"}}`
	})
	sweep("palette colour on icons", schemaEnum(t, schema, "$defs", "paletteColor", "enum"), func(v string) string {
		return `{"formatVersion": "2.0", "icon": {"format": "icon", "name": "book", "color": "` + v + `"}}`
	})
	...
	sweep("icon format", schemaEnum(t, schema, "$defs", "icon", "properties", "format", "enum"), func(v string) string {
		...
		return `{"formatVersion": "2.0", "icon": ` + icon + `}`
	})
...
func TestAuthoringSubset_IndexAndDictionaryEnumValues(t *testing.T) {
	...
	t.Run("widget layout", func(t *testing.T) {
		for _, v := range schemaEnum(t, indexSchema, "$defs", "widget", "properties", "layout", "enum") {
			requireSubsetIndex(t, `{"formatVersion": "2.0", "name": "X", "entrypoint": "page-a",
...
func TestAuthoringSubset_RefusesBackupOnlySurfaces(t *testing.T) {
	cases := map[string]string{
		"a block id":                 `{"formatVersion": "2.0", "blocks": [{"id": "b1", "type": "paragraph", "text": "x"}]}`,
		...
		"an icon by file":            `{"formatVersion": "2.0", "icon": {"format": "file", "file": "bafyreicfd"}}`,
		"an image cover":             `{"formatVersion": "2.0", "cover": {"format": "image", "file": "bafyreigejp", "y": -0.25}}`,
		"internal_key on a page":     `{"formatVersion": "2.0", "internal_key": "x"}`,
		"a counting date preset":     `{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"filters": [{"property": "p", "condition": "less", "date_preset": "number_of_days_ago", "value": 7}]}]}]}`,
		"a view id":                  `{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"id": "v1", "name": "v"}]}]}`,
		"block alignment":            `{"formatVersion": "2.0", "blocks": [{"type": "paragraph", "text": "x", "align": "center"}]}`,
		"the heading_4 input alias":  `{"formatVersion": "2.0", "blocks": [{"type": "heading_4", "text": "x"}]}`,
		"the equation input alias":   `{"formatVersion": "2.0", "blocks": [{"type": "equation", "text": "E=mc^2"}]}`,
		"a widget block":             `{"formatVersion": "2.0", "blocks": [{"type": "widget", "layout": "tree"}]}`,
		"the legacy group container": `{"formatVersion": "2.0", "blocks": [{"type": "group"}]}`,
		"a file block":               `{"formatVersion": "2.0", "blocks": [{"type": "image", "object_id": "bafyimg"}]}`,
		"a custom sort order":        `{"formatVersion": "2.0", "blocks": [{"type": "dataview", "views": [{"sorts": [{"property": "p", "direction": "custom", "custom_order": ["b", "a"]}]}]}]}`,
		"dataview output-only state": `{"formatVersion": "2.0", "blocks": [{"type": "dataview", "source": ["bafysrc"], "views": [{"name": "v"}]}]}`,
		"include_time off a date property": `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t1",
			"properties": {"name": "T"},
	...
	t.Run("member-format coupling the format refuses at semantics", func(t *testing.T) {
		...
		for name, doc := range map[string]string{
			"options off select": `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t1",
				"properties": {"name": "T"},
				"type_settings": {"property_definitions": [{"property": "p1", "format": "date", "options": ["A"]}]}}`,
			"object_types off objects/files": `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t1",
				"properties": {"name": "T"},
				"type_settings": {"property_definitions": [{"property": "p1", "format": "number", "object_types": ["task"]}]}}`,
		} {
			t.Run(name, func(t *testing.T) {
				data := []byte(doc)
				require.Error(t, Validate(data, Options{}), "validation refuses the pairing")
				require.Error(t, authoringOnly(data), "and the subset must refuse it at the schema, or it admits what the format rejects")
			})
		}
	})

	t.Run("kind alone distinguishes actual templates", func(t *testing.T) {
		// Current v2 has one rule at every door: absent kind means Page, even
		// when the ordinary type term is literally "template". The published
		// schema bytes are the bytes embedded into the package, and both compile
		// and accept that same current-format document.
		...
		kind, snapshot, err := Unmarshal(doc, Options{})
		require.NoError(t, err)
		assert.Equal(t, model.SmartBlockType_Page, kind)
		assert.Equal(t, []string{"ot-template"}, snapshot.GetObjectTypes())

		publishedPath := filepath.Join("..", "..", "format", "v2", "schema", "authoring", "object.schema.json")
		published, err := os.ReadFile(publishedPath)
		require.NoError(t, err)
		assert.Equal(t, published, AuthoringSchemaJSON(), "embedded and published authoring schemas must be identical")
		publishedSchema, err := compileStandalone(AuthoringSchemaURL, published)
		require.NoError(t, err)
		var instance any
		require.NoError(t, json.Unmarshal(doc, &instance))
		require.NoError(t, publishedSchema.Validate(instance))

		validTemplate := []byte(`{"formatVersion":"2.0","kind":"template","type":"template","template_for":"Page"}`)
		require.NoError(t, Validate(validTemplate, Options{}))
		require.NoError(t, ValidateAuthoring(validTemplate))
		for name, invalid := range map[string][]byte{
			"missing template_for": []byte(`{"formatVersion":"2.0","kind":"template","type":"template"}`),
			"missing own type":     []byte(`{"formatVersion":"2.0","kind":"template","template_for":"Page"}`),
		} {
			t.Run(name, func(t *testing.T) {
				// The full interchange grammar can represent target-less template
				// snapshots, but the authoring profile requires both fields for a
				// template an author creates from nothing.
				require.Error(t, ValidateAuthoring(invalid))
			})
		}

		legacy := []byte(`{"version":1,"type":"template"}`)
		err = ValidateAuthoring(legacy)
		require.Error(t, err)
		var validationErr *ValidationError
		require.ErrorAs(t, err, &validationErr)
		require.Len(t, validationErr.Issues, 1)
		assert.Equal(t, "/version", validationErr.Issues[0].Path)
	})
	...
	t.Run("index surfaces", func(t *testing.T) {
		...
		for name, doc := range map[string]string{
			"the manifest": `{"formatVersion": "2.0", "name": "X", "entrypoint": "p1",
				"manifest": {"properties": "properties.json"}}`,
			"the auto-widget ledger": `{"formatVersion": "2.0", "name": "X", "entrypoint": "p1",
				"auto_widget_targets": ["_bin"]}`,
			"an auto-added widget": `{"formatVersion": "2.0", "name": "X", "entrypoint": "p1",
	...
	t.Run("a dictionary entry identified only by internal_key", func(t *testing.T) {
		doc := []byte(`{"formatVersion": "2.0", "properties": [{"internal_key": "6a32d4856761631534b22f85", "format": "number"}]}`)
		_, err := UnmarshalPropertyDictionary(doc, Options{})
		require.NoError(t, err, "must be full-valid")
	...
	t.Run("dictionary member-format coupling", func(t *testing.T) {
		...
		for name, doc := range map[string]string{
			"options off select":       `{"formatVersion": "2.0", "properties": [{"property": "p1", "format": "date", "options": ["A"]}]}`,
			"object_types off objects": `{"formatVersion": "2.0", "properties": [{"property": "p1", "format": "number", "object_types": ["task"]}]}`,
			"include_time off date":    `{"formatVersion": "2.0", "properties": [{"property": "p1", "format": "text", "include_time": true}]}`,
		} {
			t.Run(name, func(t *testing.T) {
				data := []byte(doc)
				_, err := UnmarshalPropertyDictionary(data, Options{})
...
func TestAuthoringSchemas_IdentityAndHygiene(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.NotNil(t, doc.Properties.FormatVersion.Const, "schema must pin formatVersion")
			assert.Equal(t, FormatVersion, *doc.Properties.FormatVersion.Const)
...
func TestAuthoring_AllTenReservedIdsAreRefused(t *testing.T) {
	for _, id := range []string{
		...
		t.Run(id, func(t *testing.T) {
			doc := `{"formatVersion": "2.0", "id": "` + id + `", "blocks": [{"type": "paragraph", "text": "x"}]}`
	...
	t.Run("an ordinary id stays legal", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "id": "chat-notes", "blocks": [{"type": "paragraph", "text": "x"}]}`
