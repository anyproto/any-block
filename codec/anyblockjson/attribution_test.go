...
// The fixture preserves the measured 135-character shape but is generated
// from deterministic synthetic account bytes.
var testParticipantId = testfixtures.ParticipantID(testfixtures.SpaceID, testfixtures.AccountIdentity)
...
func TestAttributionProperties_DroppedNotRefused(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"creator, as a member name (what export writes)": {
			doc: `{"formatVersion": "2.0", "properties": {"creator": "Alice"}}`,
			key: "creator",
		},
		"creator, as the id array older exports wrote": {
			doc: fmt.Sprintf(`{"formatVersion": "2.0", "properties": {"creator": [%q]}}`, testParticipantId),
			key: "creator",
		},
		"last_modified_by, the spelling that used to be refused": {
			doc: `{"formatVersion": "2.0", "properties": {"last_modified_by": "Alice"}}`,
			key: "lastModifiedBy",
		},
		"last_modified_by, as an id array": {
			doc: fmt.Sprintf(`{"formatVersion": "2.0", "properties": {"last_modified_by": [%q]}}`, testParticipantId),
			key: "lastModifiedBy",
		},
		"the stored spelling drops too": {
			doc: `{"formatVersion": "2.0", "properties": {"lastModifiedBy": "Alice"}}`,
			key: "lastModifiedBy",
		},
	} {
		t.Run(name, func(t *testing.T) {
			...
			// when
			validateErr := Validate([]byte(tc.doc), Options{})
...
func TestAttributionProperties_UserChosenParticipantsAreUntouched(t *testing.T) {
	for _, key := range []string{"assignee", "author", "stakeholders"} {
		t.Run(key, func(t *testing.T) {
			// given
			doc := fmt.Sprintf(`{"formatVersion": "2.0", "properties": {%q: [%q]}}`, key, testParticipantId)
...
func TestAttributionProperties_LegendCannotLandThem(t *testing.T) {
	for name, tc := range map[string]struct{ doc, key string }{
		"creator":        {`{"formatVersion": "2.0", "property_internal_keys": {"who": "creator"}, "properties": {"who": "Alice"}}`, "creator"},
		"lastModifiedBy": {`{"formatVersion": "2.0", "property_internal_keys": {"who": "lastModifiedBy"}, "properties": {"who": "Alice"}}`, "lastModifiedBy"},
	} {
		t.Run(name, func(t *testing.T) {
			// when
			validateErr := Validate([]byte(tc.doc), Options{})
...
var (
	testAttribSpaceId  = testfixtures.SpaceID
	testAttribIdentity = testfixtures.AccountIdentity
)
...
func TestAttribution_BareIdWhenThereIsNoName(t *testing.T) {
	...
	t.Run("an empty-identity composite is omitted: it addresses nobody", func(t *testing.T) {
		...
		degenerate := testfixtures.ParticipantID(testAttribSpaceId, "")
		snap := attributionSnapshot(map[string]*types.Value{
			"creator":        strList(testParticipantId),
