...
func TestTransientProperties_DroppedNotRefused(t *testing.T) {
	for name, doc := range map[string]string{
		"empty, the shape 18,647 real objects carry": `{"formatVersion": "2.0", "properties": {"internal_flags": []}}`,
		"populated": `{"formatVersion": "2.0", "properties": {"internal_flags": ["editor_select_type"]}}`,
	} {
	...
	t.Run("a merge-resolution vector is still REFUSED, not dropped", func(t *testing.T) {
		...
		require.Error(t, Validate([]byte(`{"formatVersion": "2.0", "properties": {"oldAnytypeID": "x"}}`), Options{}))
		_, snap, err := Unmarshal([]byte(`{"formatVersion": "2.0", "properties": {"old_anytype_id": "x"}}`), Options{})
		require.NoError(t, err)
		assert.NotContains(t, snap.GetDetails().GetFields(), "oldAnytypeID")
	})

	t.Run("an ordinary property still lands", func(t *testing.T) {
		_, snap, err := Unmarshal([]byte(`{"formatVersion": "2.0", "properties": {"name": "keep me"}}`), Options{})
		require.NoError(t, err)
...
func TestTransientProperties_BundledVerdictPerKey(t *testing.T) {
	want := map[string]bool{
		...
		// vocabulary.LocalAndDerivedRelationKeys
	...
	for key, why := range transientProperties {
		t.Run(key, func(t *testing.T) {
			...
			assert.Equalf(t, verdict, vocabulary.HasRelation(domain.RelationKey(key)),
...
func TestTransientProperties_TheAnalyticsTripleIsDropped(t *testing.T) {
	...
	doc := []byte(`{"formatVersion": "2.0", "kind": "object_type", "internal_key": "use_case",
		"properties": {"name": "Use Case", "data": {"route": "SYNTHETIC_ANALYTICS_ROUTE"},
		               "isNew": true, "layoutFormat": 0}}`)

	// when
	require.NoError(t, Validate(doc, Options{}), "a stale export still imports")
...
func TestTransientProperties_FileStatusDoesNotTravel(t *testing.T) {
	t.Run("import drops, not refuses", func(t *testing.T) {
		...
		doc := []byte(`{"formatVersion": "2.0", "properties": {"name": "photo.png",
...
func TestTransientProperties_ASpacesSecretsDoNotTravel(t *testing.T) {
	secrets := map[string]string{
		"space_invite_file_key":           testfixtures.InviteFileKey,
		"space_invite_guest_file_key":     testfixtures.InviteGuestFileKey,
		"one_to_one_request_metadata_key": testfixtures.RequestMetadataKey,
		"analytics_space_id":              testfixtures.AnalyticsSpaceID,
	}
	...
	for spelling, value := range secrets {
		t.Run(spelling, func(t *testing.T) {
			...
			doc := []byte(`{"formatVersion":"2.0","kind":"space_settings","properties":{"name":"My space","` +
				spelling + `":` + strconv.Quote(value) + `}}`)
...
func TestTransientProperties_FileKeysDoNotTravel(t *testing.T) {
	snap := &model.SmartBlockSnapshotBase{
		...
		Details: fields(map[string]*types.Value{
			...
			"fileVariantKeys":      strList(testfixtures.FileVariantKey),
			"fileVariantIds":       strList(testfixtures.FileVariantID),
			"fileVariantChecksums": strList(testfixtures.FileVariantChecksum),
			"fileVariantPaths":     strList("/0/original/"),
			"fileVariantMills":     strList("/image/resize"),
			"fileVariantOptions":   strList(testfixtures.FileVariantOptions),
	...
	assert.NotContains(t, string(data), testfixtures.FileVariantKey,
	...
	t.Run("and they do not come back either", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "id": "f1", "kind": "file_object", "properties": {
			"name": "diagram.png", "file_variant_keys": ["` + testfixtures.ShortFileVariantKey + `"]}}`
		require.NoError(t, Validate([]byte(doc), Options{}))
...
func TestTransientProperties_RootBlockAnalyticsDoNotTravel(t *testing.T) {
	...
	t.Run("export strips them and keeps real state beside them", func(t *testing.T) {
		data, err := Marshal(model.SmartBlockType_Page, rootFields(map[string]*types.Value{
			"analyticsOriginalId": str(testfixtures.AnalyticsObjectID),
			"analyticsContext":    str(testfixtures.AnalyticsContext),
	...
	t.Run("a stale bundle imports without them", func(t *testing.T) {
		doc := []byte(`{"formatVersion": "2.0", "id": "o1", "root": {"fields": {
			"analyticsOriginalId": "` + testfixtures.AnalyticsObjectID + `",
			"analyticsContext": "` + testfixtures.AnalyticsContext + `", "isLocked": true}}}`)
		require.NoError(t, Validate(doc, Options{}), "a stale export still imports")
