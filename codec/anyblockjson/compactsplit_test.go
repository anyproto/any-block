...
// no legend — and CompactBlockLabels is its alias.
...
func compactSplitSnapshot() *model.SmartBlockSnapshotBase {
	return &model.SmartBlockSnapshotBase{
		...
		Blocks: []*model.Block{
			...
			textBlock("64b2c1d2e3f4a5b6c7d8e9f0", model.BlockContentText_Paragraph, "ping Alice",
...
// the three id populations that matter here (minted, hyphenated-tail,
...
func TestValidate_TheRefsLegendIsRefused(t *testing.T) {
	...
	t.Run("a retired refs member copied into a 2.0 document", func(t *testing.T) {
		// This is not a genuine pre-freeze artifact: those carry `version: 1`
		// and stop at the format gate. It models an author copying the retired
		// member into a document that declares the public 2.0 grammar.
		got := refused(t, `{"formatVersion": "2.0", "id": "bafyreiselfobjectidxxxxxxx",
			"refs": {"idxxx": "bafyreimentiontargetidxxx"},
			"blocks": [{"id": "b1", "type": "paragraph",
				"text": "ping <mention object_id=\"idxxx\">Alice</mention>"}]}`)
		...
		// And it says what happened, warning the author off the repair the bare
		// closed-set verdict suggests.
	...
	t.Run("an empty legend is refused as well", func(t *testing.T) {
		...
		// this through
		got := refused(t, `{"formatVersion": "2.0", "refs": {}}`)
		require.Len(t, got, 1, "got: %v", got)
		assert.Equal(t, "/refs", got[0].Path)
	})

	t.Run("import refuses it too, so no reader takes the labels literally", func(t *testing.T) {
		_, _, err := Unmarshal([]byte(`{"formatVersion": "2.0", "refs": {"idxxx": "bafyreitarget"},
