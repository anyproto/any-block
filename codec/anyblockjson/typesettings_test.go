...
func TestTypeSettings_FlatSpellingsAreRefusedOnTypeDocumentsOnly(t *testing.T) {
	...
	t.Run("accepted on a relation document", func(t *testing.T) {
		doc := `{"formatVersion":"2.0","kind":"property","id":"r1","internal_key":"budget",
...
func TestTypeSettings_GatedByKindWithMigrationHints(t *testing.T) {
	for name, tc := range map[string]struct{ doc, want string }{
		"type_settings on a page": {
			doc:  `{"formatVersion":"2.0","id":"o1","type_settings":{"layout":"basic"}}`,
			want: `/type_settings: property "type_settings" is only valid on type documents`,
		},
		"type_settings on a relation": {
			doc:  `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number"},"type_settings":{}}`,
			want: `property "type_settings" is only valid on type documents`,
		},
		"the pre-v0.32 root type_properties": {
			doc:  `{"formatVersion":"2.0","kind":"object_type","id":"o1","internal_key":"k","type_properties":[{"key":"due_date"}]}`,
...
func TestTypeSettings_UnknownViewNameRefusedRawNumberPasses(t *testing.T) {
	err := Validate([]byte(`{"formatVersion":"2.0","kind":"object_type","id":"t1","internal_key":"k",
		"type_settings":{"default_view":"Table"}}`), Options{})

	require.Error(t, err)
	assert.Contains(t, err.Error(), "/type_settings/default_view")
	assert.Contains(t, err.Error(), "'table'", "the refusal names the vocabulary")

	_, snap, err := Unmarshal([]byte(`{"formatVersion":"2.0","kind":"object_type","id":"t1","internal_key":"k",
...
func TestDefinitionIdentity_AKeylessDefinitionWarns(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"type document with no key": {
			`{"formatVersion":"2.0","kind":"object_type","properties":{"name":"Podcast Episode"}}`, true},
		"relation document with no key": {
			`{"formatVersion":"2.0","kind":"property","property_settings":{"format":"number"},` +
				`"properties":{"name":"Episode"}}`, true},
		"type document WITH a key": {
			`{"formatVersion":"2.0","kind":"object_type","internal_key":"podcast","properties":{"name":"Podcast"}}`, false},
		"an ordinary page owes no key": {
			`{"formatVersion":"2.0","kind":"page","properties":{"name":"A page"}}`, false},
	} {
		t.Run(name, func(t *testing.T) {
			// when
			var warned []Issue
			require.NoError(t, Validate([]byte(tc.doc), Options{OnWarning: func(i Issue) { warned = append(warned, i) }}))
...
func TestKind_TheSpacesOwnObjectSpellsItsSettings(t *testing.T) {
	...
	assert.Error(t, Validate([]byte(`{"formatVersion":"2.0","kind":"workspace","properties":{"name":"x"}}`), Options{}),
