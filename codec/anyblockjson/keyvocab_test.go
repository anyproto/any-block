...
func TestKeyVocabulary_ReverseIsATableNotACaseTransform(t *testing.T) {
	...
	t.Run("createdDate: the name says a different word than the key", func(t *testing.T) {
		...
		require.True(t, vocabulary.HasRelation(key))
...
func TestDocumentSpellsNames(t *testing.T) {
	t.Run("properties are written as display names; legacy slugs read back as stored keys", func(t *testing.T) {
		// given
		doc := `{"formatVersion": "2.0", "id": "o1", "properties": {
	...
	t.Run("a dataview's key slots follow the same vocabulary", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "id": "o1", "blocks": [{"id": "dv", "type": "dataview",
	...
	t.Run("the envelope type follows the same vocabulary", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "kind": "object_type", "id": "t1", "type": "object_type"}`
		_, snap, err := Unmarshal([]byte(doc), Options{GenerateId: seqIds("g")})
		require.NoError(t, err)
...
// (Heart's extraction-time roundtrip, recovery, and batch tools).
...
func TestImportRefusesTwoSpellingsOfOneStoredKey(t *testing.T) {
	...
	t.Run("the POST /types repro: plural_name beside pluralName", func(t *testing.T) {
		...
		doc := `{"formatVersion": "2.0", "id": "t1", "internal_key": "k",
	...
	t.Run("a vocabulary spelling over a BSON key collapses the same way", func(t *testing.T) {
		...
		doc := `{"formatVersion": "2.0", "id": "o1", "properties": {"severity": "high", "` + bsonKey + `": "low"}}`
	...
	t.Run("distinct keys are untouched", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "id": "o1", "properties": {"name": "T", "plural_name": "A", "due_date": "2025-07-06T08:44:05Z"}}`
		_, snap, err := Unmarshal([]byte(doc), Options{GenerateId: seqIds("g")})
		require.NoError(t, err)
		assert.Contains(t, snap.Details.Fields, "pluralName")
		assert.Contains(t, snap.Details.Fields, "dueDate")
