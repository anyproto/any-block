...
// un-lift (the §2b rule).
var (
	detailKeyRelationFormat            = vocabulary.RelationKeyRelationFormat.String()
	detailKeyRelationFormatIncludeTime = vocabulary.RelationKeyRelationFormatIncludeTime.String()
	detailKeyRelationFormatObjectTypes = vocabulary.RelationKeyRelationFormatObjectTypes.String()
)
...
// `relationFormatObjectTypes`, because an install rewrites bundled urls to
// the space's derived ids at creation — and stored type KEYS, which are what
// every type-key slot in this format spells (§2a, §2d). It is an optional
// capability of Options.ResolveProperties, discovered by type assertion,
// rather than a fourth resolver field: the resolver that already answers
// PropertyById is the one with the space listing this mapping falls out of
// (a space-backed resolver fills its id→key map from that same bounded
// query), and a caller without it keeps a well-defined degradation — entries
// pass through verbatim in both directions, each its own address (§3), so an
// offline round trip is byte-exact and only a resolver-wired one translates.
//
