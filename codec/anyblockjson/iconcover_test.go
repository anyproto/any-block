...
func TestImport_TheFlatSpellingsAreRefused(t *testing.T) {
	...
	for name, tc := range map[string]struct{ doc, repair string }{
		"the display name": {
			doc:    `{"formatVersion": "2.0", "properties": {"Emoji": "🔥"}}`,
			repair: `"icon": {"format": "emoji", "emoji": "…"}`,
		},
		"the stored key spelled verbatim": {
			doc:    `{"formatVersion": "2.0", "properties": {"iconImage": ["bafy"]}}`,
			repair: `"icon": {"format": "file", "file": "<object id>"}`,
		},
		"an icon name": {
			doc:    `{"formatVersion": "2.0", "properties": {"iconName": "folder"}}`,
			repair: `"icon": {"format": "icon", "name": "…"}`,
		},
		"an icon option": {
			doc:    `{"formatVersion": "2.0", "properties": {"iconOption": 3}}`,
			repair: `the "color" member of "icon"`,
		},
		"a cover id": {
			doc:    `{"formatVersion": "2.0", "properties": {"coverId": "blue"}}`,
			repair: `"cover": {"format": "image"|"color"|"gradient", …}`,
		},
		"cover framing": {
			doc:    `{"formatVersion": "2.0", "properties": {"coverY": -0.25}}`,
		...
		"a spelling the legend binds to a lifted key": {
			doc: `{"formatVersion": "2.0", "property_internal_keys": {"sneaky": "iconEmoji"},
				"properties": {"sneaky": "🔥"}}`,
			repair: `"icon": {"format": "emoji", "emoji": "…"}`,
		},
	} {
	...
	t.Run("the retired flat slug is an ordinary custom key", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "properties": {"icon_emoji": "🔥"}}`
...
func TestRoundTrip_CalloutIcon(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NoError(t, Validate(first, Options{}), "%s", first)
	...
	t.Run("the four-variant object icon is not a callout icon", func(t *testing.T) {
		err := Validate([]byte(`{"formatVersion": "2.0", "blocks": [
			{"type": "callout", "icon": {"format": "icon", "name": "folder"}, "text": "x"}]}`), Options{})
...
func TestValidate_AWrongIconGetsExactlyOneIssue(t *testing.T) {
	for _, tc := range []struct{ doc, path, message string }{
		...
		{`{"formatVersion":"2.0","icon":{"format":"file","file":"https://images.example/hero.jpg"}}`,
			"/icon/file", `does not match pattern '^[^/]+$'`},
		{`{"formatVersion":"2.0","cover":{"format":"image","file":"/var/folders/j0/T/x.png"}}`,
			"/cover/file", `does not match pattern '^[^/]+$'`},
		{`{"formatVersion":"2.0","cover":{"format":"unsplash","file":"bafy"}}`,
			"/cover/format", "value must be one of 'image', 'color', 'gradient'"},
		{`{"formatVersion":"2.0","cover":{"gradient":"pinkOrange"}}`,
			"/cover", "a cover is one of 'image', 'color', 'gradient'"},
		{`{"formatVersion":"2.0","blocks":[{"type":"callout","icon":{"emoji":"💡"},"text":"x"}]}`,
...
// (Heart's core/acl/aclservice.go writes SpaceIconCid into iconImage, whose format is
...
func TestIcon_FileHoldsEitherAnObjectIdOrAContentCid(t *testing.T) {
	for _, tc := range []struct{ name, id string }{
		{"an object id", testfixtures.ObjectID},
		{"a raw content cid, as a participant avatar carries", testfixtures.ContentID},
	} {
		t.Run(tc.name, func(t *testing.T) {
			doc := `{"formatVersion": "2.0", "id": "o1", "icon": {"format": "file", "file": "` + tc.id + `"}}`
			require.NoError(t, Validate([]byte(doc), Options{}))
			_, snap, err := Unmarshal([]byte(doc), Options{GenerateId: seqIds("g")})
			require.NoError(t, err)
