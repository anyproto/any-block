...
type Options struct {
	...
	// header name to the column id accepted by table edit operations.
	TableColumnHeaders bool
	// Keys supplies the spelling ↔ stored-key vocabulary. Nil selects
	// BundledKeyVocabulary. Validate and Unmarshal agree exactly for Options{}
	// or an equivalent non-widening bundled vocabulary; a wider/store-backed
	// vocabulary may add path-addressed semantic admission refusals that
	// byte-only Validate cannot predict.
	Keys KeyVocabulary
...
func (o Options) compactBlockLabels() bool { return o.CompactBlockLabels }
...
func MarshalPropertyValue(key string, v *types.Value, opts Options) (any, map[string]string) {
	out, ids, err := MarshalPropertyValueChecked(key, v, opts)
	if err != nil {
		// The legacy signature has no error result. Return a non-null JSON
		// marshaler that preserves the refusal until the caller's normal
		// encoding step; this cannot be mistaken for an authored JSON null.
		return refusedJSONValue{err: err}, ids
	}
	return out, ids
}

// MarshalPropertyValueChecked is the lossless value-level writer. Every
// protobuf number is already a float64, so finite values are stable; the
// recursive check rejects NaN/Inf with a pointer relative to v. It is the
// preferred compatibility path for callers that need an immediate error.
...
type refusedJSONValue struct{ err error }

func (v refusedJSONValue) MarshalJSON() ([]byte, error) { return nil, v.err }
...
type exporter struct {
	...
	fragmentRoot bool
	// emissionPlan is the side-effect-free structural census shared by table
	// bounds, axis emission, and derived-cell reservation.
	emissionPlan *exportEmissionPlan
...
//
// Only blocks the canonical emission plan actually serves are reserved. A
// snapshot's block list is not its rendered tree: orphaned subtrees survive,
// leaf blocks can carry stale children, and shared table axes belong only to
// the first table that emits them. Reserving any of those invisible ids would
// rename a valid visible block and break the export/import/export fixed point.
func (e *exporter) seedIdLabels() {
	...
	for _, b := range e.snapshot.Blocks {
		if b == nil || b.Id == "" || seen[b.Id] || !plan.emitted[b.Id] {
			continue
		}
		seen[b.Id] = true
...
// derivedCellIds lists the cell id every emitted table's owned axes imply:
// rowLabel + "-" + colLabel for the whole effective grid, materialized or not
// (§6.1). It runs after every surviving row and column has its label and reads
// the exact population tableToJSON renders. Hidden tables and pairs containing
// an axis consumed by an earlier table reserve nothing.
func (e *exporter) derivedCellIds() []string {
	plan := e.canonicalEmissionPlan()
	var out []string
	for _, tableID := range plan.tableOrder {
		axes := plan.tables[tableID]
		if !tableGridWithinLimit(len(axes.rows), len(axes.columns)) {
			// The renderer reports the bound before entering the Cartesian
			// work. Do not duplicate that work just to prepare labels for an
			// export that will be refused.
			continue
		}
		columnBlocks, rowBlocks := axes.columns, axes.rows
		cols := make([]string, 0, len(columnBlocks))
		for _, column := range columnBlocks {
			if label := e.idLabels[column.Id]; label != "" {
				cols = append(cols, label)
			}
		}
		rows := make([]string, 0, len(rowBlocks))
		for _, row := range rowBlocks {
			if label := e.idLabels[row.Id]; label != "" {
...
// checkTableGridBounds rejects an emitted table before either tableToJSON or
// derivedCellIds enters its row×column work. The population is the canonical
// emission plan's globally owned axes, so a table hidden below a leaf counts
// nothing and an axis already consumed by an earlier table is absent here as
// well as in the renderer.
func (e *exporter) checkTableGridBounds() error {
	plan := e.canonicalEmissionPlan()
	for _, tableID := range plan.tableOrder {
		axes := plan.tables[tableID]
		if !tableGridWithinLimit(len(axes.rows), len(axes.columns)) {
			return fmt.Errorf("table %s: grid has %d rows × %d columns; the maximum implicit cell grid is %d",
				tableID, len(axes.rows), len(axes.columns), maxTableGridCells)
		}
	}
	return nil
}

type tableEmissionAxes struct {
	columns []*model.Block
	rows    []*model.Block
}

type exportEmissionPlan struct {
	emitted    map[string]bool
	tables     map[string]tableEmissionAxes
	tableOrder []string
}

// canonicalEmissionPlan is a side-effect-free walk of the block emission
// shape. It mirrors the pre-order, leaf stopping, transparent lifting,
// top-level structural dropping, and global emit-once rules that decide
// whether tableToJSON is reached. At a table it additionally performs the
// column-then-row claims tableToJSON makes, preserving the exact surviving
// axes for every downstream consumer.
func (e *exporter) canonicalEmissionPlan() *exportEmissionPlan {
	if e.emissionPlan != nil {
		return e.emissionPlan
	}
	plan := &exportEmissionPlan{
		emitted: map[string]bool{},
		tables:  map[string]tableEmissionAxes{},
	}
	e.emissionPlan = plan
	...
	var walkIds func([]string, bool)
	var walkBlock func(*model.Block, bool, bool)
	var walkCell func(*model.Block)

	walkIds = func(ids []string, topLevel bool) {
		for _, id := range ids {
			walkBlock(e.blocks[id], topLevel, true)
		}
	}
	walkCell = func(cell *model.Block) {
		if cell == nil {
			return
		}
		// Unlike an ordinary transparent container, a transparent cell has no
		// run to lift into: cellToJSON consumes it and drops its descendants.
		if isTransparentContainer(cell) {
			if cell.Id != "" && !visited[cell.Id] {
				visited[cell.Id] = true
			}
			return
		}
		walkBlock(cell, false, false)
	}
	walkBlock = func(b *model.Block, topLevel, record bool) {
		if b == nil {
			return
		}
		...
		if isTransparentContainer(b) {
			...
			walkIds(b.ChildrenIds, topLevel)
			return
		}
		if b.Id != "" {
			if visited[b.Id] {
		...
		emits, descends := blockEmissionShape(b)
		if !emits {
			return
		}
		if record && b.Id != "" {
			plan.emitted[b.Id] = true
		}

		if _, isTable := b.Content.(*model.BlockContentOfTable); isTable {
			_, _, rawColumns, rawRows := e.tableAxes(b)
			axes := tableEmissionAxes{}
			for _, column := range rawColumns {
				if column.Id == "" || visited[column.Id] {
					continue
				}
				visited[column.Id] = true
				plan.emitted[column.Id] = true
				axes.columns = append(axes.columns, column)
			}
			for _, row := range rawRows {
				if row.Id == "" || visited[row.Id] {
					continue
				}
				visited[row.Id] = true
				plan.emitted[row.Id] = true
				axes.rows = append(axes.rows, row)
			}
			plan.tables[b.Id] = axes
			plan.tableOrder = append(plan.tableOrder, b.Id)

			// Actual rendering stops at the product guard. Cell visitation can
			// influence later emit-once claims only for a table that passes it.
			if !tableGridWithinLimit(len(axes.rows), len(axes.columns)) {
				return
			}
			rows := append([]*model.Block(nil), axes.rows...)
			sort.SliceStable(rows, func(i, j int) bool {
				left := orEmpty(rows[i].Content.(*model.BlockContentOfTableRow).TableRow).IsHeader
				right := orEmpty(rows[j].Content.(*model.BlockContentOfTableRow).TableRow).IsHeader
				return left && !right
			})
			for _, row := range rows {
				byColumn := map[string]*model.Block{}
				for _, cellID := range row.ChildrenIds {
					columnID, ok := strings.CutPrefix(cellID, row.Id+"-")
					if ok {
						byColumn[columnID] = e.blocks[cellID]
					}
				}
				for _, column := range axes.columns {
					walkCell(byColumn[column.Id])
				}
			}
			return
		}
		if descends {
			walkIds(b.ChildrenIds, false)
		}
	}

	if e.fragmentRoot {
		walkIds([]string{e.rootId}, false)
	} else if root := e.blocks[e.rootId]; root != nil {
		walkIds(root.ChildrenIds, true)
	}
	return plan
}

// blockEmissionShape mirrors only blockToJSON's emit/drop and child-descent
// decisions. It intentionally does no rendering, resolution, warning, or ID
// allocation, so preflight table ownership cannot duplicate caller-visible
// side effects.
func blockEmissionShape(b *model.Block) (emits, descends bool) {
	...
	case *model.BlockContentOfLayout:
		style := orEmpty(content.Layout).Style
		return style == model.BlockContentLayout_Row || style == model.BlockContentLayout_Column,
			style == model.BlockContentLayout_Row || style == model.BlockContentLayout_Column
	case *model.BlockContentOfRelation:
		key := orEmpty(content.Relation).Key
		return key != "" && isWritablePropertyKey(key), false
...
func (e *exporter) buildDoc(sbType model.SmartBlockType) (*omap, error) {
	...
	// reading declares legacy `version: 1`, which the version gate refuses for the
	...
	typeSettings, err := e.buildTypeSettings()
	if err != nil {
		return nil, err
	}
...
// for tooling that has to agree with that set — a round-trip checker
// comparing a snapshot with its re-import has to know which keys are
// *expected* to be gone, and asking here is the only way to stay in
// agreement. That is not a nicety: while separate copies of the list
// existed, they drifted, and each one reported 10 378 false data-loss issues
// over a 36 808-object account the moment the package added the importer's
// provenance keys.
...
func strippedDetailKeys() map[string]bool {
	...
	for _, k := range vocabulary.LocalAndDerivedRelationKeys {
...
func (e *exporter) buildProperties() *omap {
	...
	for _, p := range ordered {
		...
		m.set(p.slug, e.propertyValue(p.key, p.slug, e.snapshot.Details.Fields[p.key]))
	}
	return m
}
...
func resolveFormatWith(opts Options, key string) (model.RelationFormat, bool) {
	if f, err := vocabulary.GetRelationFormat(domain.RelationKey(key)); err == nil {
		return f, true
	}
...
func (e *exporter) emittedLocalIds() map[string]bool {
	...
	for _, id := range probe.derivedCellIds() {
