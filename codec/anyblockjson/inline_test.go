...
func TestRenderInline_Golden(t *testing.T) {
	tests := []struct {
		...
		{"mention", "ping Alice", []*model.BlockContentTextMark{mark(mMention, 5, 10, "bafyid")}, `ping <mention object_id="bafyid">Alice</mention>`},
...
// namespace reserves (§8.2, §10). Escaping only the three tags formatVersion 2.0
...
// tell 2.0-literal from 2.1-markup from the text string alone. So the escape is
...
func TestParseInline_Golden(t *testing.T) {
	tests := []struct {
		...
		{"mention", `<mention object_id="id1">Alice</mention>`, "Alice",
