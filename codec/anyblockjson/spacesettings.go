...
var spaceSettingsIndexKeys = map[string]string{
	vocabulary.RelationKeyName.String():        "name",
	vocabulary.RelationKeyDescription.String(): "description",
	"homepage":          "homepage",
...
func IndexFromSpaceSettings(idx *Index, base *model.SmartBlockSnapshotBase) {
	...
	det := base.GetDetails().GetFields()
	if v := stringDetail(det, vocabulary.RelationKeyName.String()); v != "" {
		idx.Name = v
	}
	if v := stringDetail(det, vocabulary.RelationKeyDescription.String()); v != "" {
		idx.Description = v
	}
...
func OmittedSpaceSettings(sbType model.SmartBlockType, base *model.SmartBlockSnapshotBase) bool {
	...
	for k := range base.GetDetails().GetFields() {
		...
		case k == vocabulary.RelationKeyFeaturedRelations.String():
		...
		case k == vocabulary.RelationKeyCreatedDate.String(),
			k == vocabulary.RelationKeyLastModifiedDate.String():
...
var spaceSettingsConstantKeys = map[string]bool{
	vocabulary.RelationKeyLayout.String():                 true, // "space", 1 distinct
	vocabulary.RelationKeyResolvedLayout.String():         true, // "dashboard", 1 distinct
	vocabulary.RelationKeyIsHidden.String():               true, // true, 1 distinct
	vocabulary.RelationKeyMigrationObjectContext.String(): true, // 10, 1 distinct
	"migrationBackRelations":                              true, // 1 distinct
	vocabulary.RelationKeyId.String():                     true, // the envelope carries it
	vocabulary.RelationKeySpaceId.String():                true, // the bundle IS the space
}
