...
// `codec/anyblockjson/vocabulary`'s relations.json/types.json, so `createdDate` spells
...
//
//   - the derived api-slug table (`vocabulary.ApiSlug`, i.e. strcase.ToSnake):
...
// the api surface's vocabulary.FoldApiKey on purpose: a display name separates
...
// guess — the same contract vocabulary.FoldApiKey states.
//
...
func init() {
	...
	for _, k := range vocabulary.ListRelationsKeys() {
	...
	for _, k := range vocabulary.ListTypesKeys() {
		typeKeys = append(typeKeys, string(k))
	}
	sort.Strings(typeKeys)
	typeName := func(key string) string {
		t, err := vocabulary.GetType(domain.TypeKey(key))
		if err != nil {
			return ""
		}
		return t.Name
...
// vocabulary. Keys arrive SORTED so that where the input is ambiguous the
...
// fold claimants included — before it may answer. It is exported so that a
// space-backed vocabulary's exact candidate layer can run this same table:
// folding inside it would let a bundled near-miss win while a space-minted
// twin in the same fold class went unseen.
...
// refuse rather than resolve. It is exported so that a space-backed
// vocabulary's chain runs the same bundled arm a package-only reader does:
// the two must not disagree about which fold class a key answers to.
...
// synthetic. It is exported because the exporter's per-document term ledger
// and a space-backed vocabulary both run this ladder, and a claimant must
// take the same spelling whichever seam degrades it.
