...
func TestPropertyDefinitions_InternalKeyAloneIdentifiesVerbatim(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			// given
			doc := `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t",
...
func TestPropertyDefinitions_IdentityIsPropertyOrInternalKeyOrName(t *testing.T) {
	entryDoc := func(entry string) string {
		return `{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t",
	...
	t.Run("each identity member alone is enough", func(t *testing.T) {
		for _, entry := range []string{
			...
			assert.NoError(t, Validate([]byte(entryDoc(entry)), Options{}), entry)
		}
	})
	t.Run("an entry with no identity at all is refused", func(t *testing.T) {
		err := Validate([]byte(entryDoc(`{"format": "number"}`)), Options{})
		require.Error(t, err)
	})
	t.Run("the dictionary home says the same", func(t *testing.T) {
		_, err := UnmarshalPropertyDictionary([]byte(
			`{"formatVersion":"2.0","properties":[{"internal_key":"6a83296f61fab2265263ae34","format":"number"}]}`), Options{})

		assert.NoError(t, err)
		_, err = UnmarshalPropertyDictionary([]byte(
			`{"formatVersion":"2.0","properties":[{"format":"number"}]}`), Options{})
...
func TestPropertyDictionary_InternalKeyIsVerbatim(t *testing.T) {
	d, err := UnmarshalPropertyDictionary([]byte(`{"formatVersion":"2.0","properties":[
		{"internal_key":"due_date","format":"date"},
		{"internal_key":"6a83296f61fab2265263ae34","name":"Budget","format":"number"}]}`), Options{})
...
func TestPropertyDictionary_DuplicateAcrossTheTwoIdentityMembers(t *testing.T) {
	_, err := UnmarshalPropertyDictionary([]byte(`{"formatVersion":"2.0","properties":[
		...
		{"internal_key":"6a83296f61fab2265263ae34","format":"text"}]}`), Options{})
...
func TestRelationSettings_RefusesTheIdentityPair(t *testing.T) {
	for member, wantHome := range map[string]string{
		...
		t.Run(member, func(t *testing.T) {
			err := Validate([]byte(`{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b",
				"property_settings":{"format":"number","`+member+`":"x"}}`), Options{})
...
func TestPropertyDefinitions_UnwritableInternalKeyIsRefusedByName(t *testing.T) {
	err := Validate([]byte(`{"formatVersion": "2.0", "kind": "object_type", "internal_key": "t",
		"type_settings": {"property_definitions": [{"internal_key": "a\nb", "format": "text"}]}}`), Options{})
