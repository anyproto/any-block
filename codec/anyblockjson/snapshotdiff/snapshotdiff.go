...
// text-multiset comparator a round-trip harness reports with, and the metric
// behind a backtranslation eval. Findings are triage input, not proof.
...
// vocabulary.SystemRelations — see systemtrim.go for the admission test each key
...
var recommendedListKeys = map[string]bool{
	vocabulary.RelationKeyRecommendedFeaturedRelations.String(): true,
	vocabulary.RelationKeyRecommendedRelations.String():         true,
	vocabulary.RelationKeyRecommendedFileRelations.String():     true,
	vocabulary.RelationKeyRecommendedHiddenRelations.String():   true,
}
...
func Compare(orig, got *model.SmartBlockSnapshotBase, sbType model.SmartBlockType, opts anyblockjson.Options) []string {
	...
	if orig.Details != nil {
		...
		for _, k := range keys {
			...
			if gotFields[k] == nil && k == vocabulary.RelationKeyIconImage.String() &&
...
var relationTargetsDetailKey = vocabulary.RelationKeyRelationFormatObjectTypes.String()
...
func normalizeTypeRefs(entries []string, tr anyblockjson.TypeResolver, opts anyblockjson.Options) []string {
	...
	for _, entry := range entries {
		if key, err := vocabulary.TypeKeyFromUrl(entry); err == nil && key != "" {
...
func normalizeRecommended(entries []string, r anyblockjson.PropertyResolver) []string {
	...
	for _, id := range entries {
		...
		if _, err := vocabulary.GetRelation(domain.RelationKey(id)); err == nil {
