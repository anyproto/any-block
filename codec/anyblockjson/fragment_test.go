...
func TestUnmarshalBlocks(t *testing.T) {
	...
	t.Run("authored ids later in the run are reserved before generation", func(t *testing.T) {
		blocks, topIds, err := UnmarshalBlocks(rawRun(
			`{"type":"paragraph","text":"generated"}`,
			`{"id":"taken","type":"paragraph","text":"authored"}`,
		), Options{GenerateId: func() string { return "taken" }})

		require.NoError(t, err)
		require.Len(t, blocks, 2)
		assert.Equal(t, []string{"taken_2", "taken"}, topIds)
		assert.Equal(t, "taken_2", blocks[0].Id)
		assert.Equal(t, "taken", blocks[1].Id)
	})
	...
	t.Run("duplicate members are rejected before fragment decoding", func(t *testing.T) {
		_, _, err := UnmarshalBlocks(rawRun(
			`{"type":"paragraph","text":"first"}`,
			`{"type":"paragraph","text":"second","text":"replacement"}`,
		), Options{})

		var ve *ValidationError
		require.ErrorAs(t, err, &ve)
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/blocks/1/text", ve.Issues[0].Path)
		assert.Contains(t, ve.Issues[0].Message, `duplicate object member "text"`)
	})
	...
	t.Run("deferred key-slot refusals are finalized", func(t *testing.T) {
		_, _, err := UnmarshalBlocks(rawRun(
			`{"type":"property","property":"blank"}`), Options{Keys: blankKeyVocab{}})

		var ve *ValidationError
		require.ErrorAs(t, err, &ve)
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/blocks", ve.Issues[0].Path)
	})
}
...
func TestUnmarshalBlock(t *testing.T) {
	...
	t.Run("forcedId is reserved before owned block ids are generated", func(t *testing.T) {
		blocks, err := UnmarshalBlock(json.RawMessage(
			`{"type":"table","columns":[{"id":"column"}],"rows":[]}`),
			"taken", Options{GenerateId: func() string { return "taken" }})

		require.NoError(t, err)
		require.NotEmpty(t, blocks)
		seen := map[string]bool{}
		for _, block := range blocks {
			require.False(t, seen[block.Id], "duplicate block id %q", block.Id)
			seen[block.Id] = true
		}
		assert.Equal(t, "taken", blocks[0].Id)
		assert.True(t, seen["taken_2"], "the first generated wrapper avoids the forced id")
	})
	...
	t.Run("deferred key-slot refusals are finalized", func(t *testing.T) {
		...
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/blocks", ve.Issues[0].Path)
		assert.Contains(t, ve.Issues[0].Message, "resolves the property block `property` spelling")
	})
	...
	t.Run("duplicate members are rejected before single-block decoding", func(t *testing.T) {
		_, err := UnmarshalBlock(json.RawMessage(
			`{"type":"paragraph","type":"quote","text":"x"}`), "", Options{})

		var ve *ValidationError
		require.ErrorAs(t, err, &ve)
		require.Len(t, ve.Issues, 1)
		assert.Equal(t, "/blocks/0/type", ve.Issues[0].Path)
		assert.Contains(t, ve.Issues[0].Message, `duplicate object member "type"`)
	})
}
...
func TestMarshalBlockSubtree_ObjectRefsAreNeverCompacted(t *testing.T) {
	...
	subtree := []*model.Block{{Id: "64b2c1d2e3f4a5b6c7d8e9f0",
		Content: &model.BlockContentOfText{Text: &model.BlockContentText{
			Text: "ping Alice", Marks: &model.BlockContentTextMarks{
