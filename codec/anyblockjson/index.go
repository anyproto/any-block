...
var indexSchemaJSON = formatschema.Index()
...
//
// That disjointness is the whole point, and it is not decorative. A pb
// importer resolves a widget's target through the bundle's own id map FIRST,
// and only then asks whether the target names a built-in listing. While the
// reserved listings were bare words — `set`, `favorite` — a bundle shipping
// an object with id `set` silently captured the widget that meant *the Sets
// listing*, and Index.EntryPoint, which skips reserved targets, silently
// disagreed with EffectiveEntryPoint about what the bundle even opens. A
// prefix rule needs only "no minted id STARTS with `_`", which is
// permanently true, where a per-word reservation needs a new ban every time
// a listing is added.
...
//
// These are the format's spellings, not the wire's: the store carries them
// as the bare `widgets` / `graph`, and the installer switches on those names
// BEFORE looking an id up — the opposite precedence from widget targets, and
// the reason a bundle object with id `graph` can never be a homepage. Both
// directions close once the reserved spellings live in the platform
// namespace. The `_` is translated away at the wire boundary, by
// WireHomepage.
const (
...
// spelling the store uses for the same screen. WireHomepage walks it one way
// and FormatHomepage the other: the STORE holds the wire spelling, so a
// value lifted out of a space object has to be translated before it becomes
// an index field, exactly as one written to a profile has to be translated
// on the way out.
...
// anything else.
//
// The translation is not cosmetic and is the reason the rename is safe. An
// importer leaves a link target alone only when it recognises the target as
// a predefined listing, and what it recognises are the bare wire words and
// nothing else — write `_set` and the link is rewritten to the importer's
// missing-object placeholder and then stripped along with its wrapper,
// losing the widget with no error. A reserved-but-not-importable target has
// no wire spelling at all, and is returned unchanged so it fails loudly
// rather than impersonating an id; a converter is expected to refuse those
// before conversion, which IsImportableWidgetTarget is the test for.
...
// FormatHomepage is WireHomepage's inverse: the format's `_`-prefixed
// spelling for a bare wire screen name, the value itself for anything else.
// It is what a lift out of a stored space object applies to the homepage, so
// the stored `graph` becomes the `_graph` no bundle object may claim (§1).
...
// WireHomepage returns the value the wire's space-dashboard field should
// carry for a homepage: the bare screen name for a reserved one, the object
// id otherwise. The installer matches those bare names before it tries to
// resolve an id, so an untranslated `_graph` would be looked up as an
// object, fail, and fall back to the widgets screen.
...
// namespace is the format's own guarantee (§1). But the wire spellings are
// BARE — WireWidgetTarget translates `_set` to `set` on the way out — and an
// importer resolves a link target through the bundle's id map before it asks
// whether the target names a built-in listing. So an object with id `set`
// captures the translated widget exactly the way it captured the
// untranslated one: renaming the format's spelling moves the collision
// downstream rather than removing it, unless the wire spellings are
// unmintable too.
//
// The homepages are the same story with the precedence reversed: the
// installer matches `graph` before resolving an id, so a bundle object with
// that id is simply unreachable as a homepage.
...
type Index struct {
	...
	//
	// The installer resolves the space icon by image *name* — it queries for
	// an object with that name and an image layout — so the wiring looks the
	// name up from this id; that asymmetry is the wire format's, not the
...
// Unmarshal.
//
// Whether the ids it names exist is a cross-document question: an index is
// valid on its own terms while pointing at an object no document defines.
// The repository-root bundle.Validate function checks those references.
// Warning-grade issues, including the mechanical migration from pre-release
// `version: 2` to public `formatVersion: "2.0"`, go to Options.OnWarning.
func UnmarshalIndex(data []byte, opts Options) (*Index, error) {
	...
	// a generic const failure that says nothing about why.
	migrated, err := checkVersion(doc, onWarning)
	if err != nil {
		return nil, err
	}
	...
	if issues := indexWidgetPropertyCollisionIssues(doc); len(issues) > 0 {
		return nil, &ValidationError{Issues: issues}
	}
...
// widget that the reader would otherwise collapse to the same stored key.
// Array order is the claimant order, so the later position is both stable and
// the precise repair location.
func indexWidgetPropertyCollisionIssues(doc map[string]any) []Issue {
	widgets, _ := doc["widgets"].([]any)
	var issues []Issue
	for widgetIndex, rawWidget := range widgets {
		widget, _ := rawWidget.(map[string]any)
		properties, _ := widget["properties"].([]any)
		type claimant struct {
			spelling string
			index    int
		}
		firstByStored := make(map[string]claimant, len(properties))
		for propertyIndex, rawProperty := range properties {
			spelling, _ := rawProperty.(string)
			stored, _ := dictionaryStoredKey(spelling)
			if first, exists := firstByStored[stored]; exists {
				if first.spelling != spelling {
					issues = append(issues, Issue{
						Path: fmt.Sprintf("/widgets/%d/properties/%d", widgetIndex, propertyIndex),
						Message: fmt.Sprintf("widget property spellings %q at /widgets/%d/properties/%d and %q both resolve to stored key %q; keep one spelling",
							first.spelling, widgetIndex, first.index, spelling, stored),
					})
				}
				continue
			}
...
// recovers that exact key from the canonical wire spelling.
func indexWidgetPropertyBinding(stored string) (wire, effective string, fixed bool) {
	wire = dictionaryKeySpelling(stored)
	effective, _ = dictionaryStoredKey(wire)
...
func MarshalIndex(idx *Index) ([]byte, error) {
	...
	for _, w := range idx.Widgets {
		...
		destinationIndex := len(widgets)
		propertySpellings := make([]string, len(w.Properties))
		for propertyIndex, stored := range w.Properties {
			wire, effective, fixed := indexWidgetPropertyBinding(stored)
			if !fixed {
				return nil, &ValidationError{Issues: []Issue{{
					Path: fmt.Sprintf("/widgets/%d/properties/%d", destinationIndex, propertyIndex),
					Message: fmt.Sprintf("stored widget property key %q writes as %q, which reads as stored key %q; "+
						"the index cannot preserve this property binding", stored, wire, effective),
				}}}
			}
		...
		wm.setNonEmpty("properties", stringsToAny(propertySpellings))
