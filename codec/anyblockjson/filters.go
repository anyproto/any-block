...
func UnmarshalFilters(raw json.RawMessage, opts Options) ([]*model.BlockContentDataviewFilter, error) {
	// Raw ambiguity owns the first verdict. Both tolerant decoders below and
	// the synthetic-document wrapper otherwise observe only one survivor of a
	// duplicated member, so a semantic fault in that survivor could mask the
	// duplicate. Keep this pass separate from validateQueryFragment: moving
	// the whole schema pass here would take vocabulary wording away from
	// duplicate-free input.
	if err := preflightQueryFragment("filters", raw); err != nil {
		return nil, err
	}
	...
	if err := jsonUnmarshal(raw, &nodes); err != nil {
		...
		if fragErr := validateQueryFragment("filters", raw, opts); isValidationError(fragErr) {
			return nil, fragErr
		}
	...
	var localWarnings []Issue
	warnIssue := func(path, format string, args ...any) {
		if opts.OnWarning != nil {
			localWarnings = append(localWarnings, Issue{Path: path, Message: fmt.Sprintf(format, args...)})
		}
	}
	...
	if err := validateQueryFragment("filters", raw, opts); err != nil {
		return nil, err
	}
	// Whole-document validation visits envelope legends before dataview
	// semantics. The fragment's local date pass needs the caller's format
	// resolver, so buffer its warnings until the synthetic envelope pass has
	// emitted the intrinsic legend warnings in that same order.
	...
	if err := imp.finalize("/filters"); err != nil {
		return nil, err
	}
	return out, nil
}
...
func UnmarshalSorts(raw json.RawMessage, opts Options) ([]*model.BlockContentDataviewSort, error) {
	if err := preflightQueryFragment("sorts", raw); err != nil {
		return nil, err
	}
	var sorts []jsonSort
	if err := jsonUnmarshal(raw, &sorts); err != nil {
		...
		if fragErr := validateQueryFragment("sorts", raw, opts); isValidationError(fragErr) {
			return nil, fragErr
		}
	...
	if err := validateQueryFragment("sorts", raw, opts); err != nil {
		return nil, err
	}
	...
	if err := imp.finalize("/sorts"); err != nil {
		return nil, err
	}
	return out, nil
}
...
// about the supplied envelope legend are forwarded. Warnings below the
// synthetic dataview are discarded because the fragment entry points run
// that semantic pass themselves with the caller's format resolver.
func validateQueryFragment(member string, raw json.RawMessage, opts Options) error {
	doc := map[string]any{
	...
	warn := func(issue Issue) {
		if opts.OnWarning == nil {
			return
		}
		const syntheticView = "/blocks/0/views/0"
		if issue.Path == syntheticView || strings.HasPrefix(issue.Path, syntheticView+"/") {
			return
		}
		opts.OnWarning(issue)
	}
	if _, err := validatePartialToDoc(payload, false, warn); err != nil {
...
// preflightQueryFragment applies the raw JSON admission pass before either
// query decoder can collapse duplicate object members. strictJSONDocumentPreflight
// sees a bare array, so prefix its array-relative pointers with the public
// fragment member that the caller supplied.
func preflightQueryFragment(member string, raw json.RawMessage) error {
	if err := strictJSONDocumentPreflight(raw); err != nil {
		var ve *ValidationError
		if !errors.As(err, &ve) {
			return err
		}
		issues := make([]Issue, len(ve.Issues))
		for i, issue := range ve.Issues {
			issue.Path = "/" + member + issue.Path
			issues[i] = issue
		}
