...
func participantSnapshot() *model.SmartBlockSnapshotBase {
	return &model.SmartBlockSnapshotBase{
		Details: fields(map[string]*types.Value{
			"id":               str(testfixtures.AccountIdentity),
			"name":             str("Alice"),
...
func TestParticipantProvenance_DroppedNotRefusedOnImport(t *testing.T) {
	doc := `{"formatVersion": "2.0", "kind": "participant",
		"id": "` + testfixtures.AccountIdentity + `",
		"properties": {"name": "Alice", "created_date": "2026-08-26T21:42:26Z"}}`

	require.NoError(t, Validate([]byte(doc), Options{}), "stale, not wrong")
	...
	assert.Equal(t, "Alice", snap.GetDetails().GetFields()["name"].GetStringValue())

	t.Run("on a page the same property still lands", func(t *testing.T) {
		doc := `{"formatVersion": "2.0", "properties": {"name": "A page", "created_date": "2026-08-26T21:42:26Z"}}`
