...
func TestOptionRefs_NameCarryingASpace(t *testing.T) {
	...
	// then
	require.NoError(t, Validate(data, Options{}), "a name with a space must be a legal legend key:\n%s", data)
...
func TestOptionRefs_UnknownValuePassesThrough(t *testing.T) {
	...
	doc := `{"formatVersion": "2.0", "id": "obj1", "properties": {"tag": ["Brand new"]},
...
func TestOptionRefs_OverLongNameStillGetsAnEntry(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// then
			require.NoError(t, Validate(data, Options{}), "%s", data)
...
func TestOptionRefs_ReaderWithoutAResolverIgnoresTheLegend(t *testing.T) {
	// given
	doc := `{"formatVersion": "2.0", "id": "obj1", "properties": {"tag": ["High"]},
		"option_ids": {"tag": {"High": "bafyopt"}}}`
...
func TestOptionRefs_LegendKeyRulesAreTheSameInBothValidators(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// the census
			schemaErr := Validate(raw, Options{OnWarning: func(Issue) {}})
...
func TestOptionRefs_ALegendEntryForAPropertyTheDocumentDoesNotUse(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// given
			doc := fmt.Sprintf(`{"formatVersion": "2.0", "id": "obj1", "properties": {"tag": ["High"]},
				"option_ids": {%q: {"High": "bafylegend"}}}`, tc.slug)

			// when
			var warned []Issue
			validateErr := Validate([]byte(doc), Options{OnWarning: func(i Issue) { warned = append(warned, i) }})
			_, back, err := Unmarshal([]byte(doc), Options{ResolveOptions: space, GenerateId: seqIds("g")})
...
func TestOptionRefs_PropertySpelledOnlyInsideADataview(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// given
			doc := fmt.Sprintf(`{"formatVersion": "2.0", "id": "obj1",
...
func TestOptionRefs_ThePropertyCensusCoversEveryPosition(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			// given
			data := []byte(`{"formatVersion": "2.0", "id": "obj1",` + strings.TrimPrefix(tc.doc, "{"))
...
// recordingOptions counts which half of OptionResolver a direction calls.
// The interface documents two duties per method, and the only way to know
// they are the real ones is to watch a codec run ask.
type recordingOptions struct {
	space    spaceOptions
	names    []string // OptionId asked about
	ids      []string // OptionName asked about
	answerId bool     // false makes this an export-only resolver
}

func (r *recordingOptions) OptionName(key domain.RelationKey, id string) (string, bool) {
	r.ids = append(r.ids, id)
	return r.space.OptionName(key, id)
}

func (r *recordingOptions) OptionId(key domain.RelationKey, name string) (string, bool) {
	r.names = append(r.names, name)
	if !r.answerId {
		return "", false
	}
	return r.space.OptionId(key, name)
}

// EXPORT ASKS OptionId TOO, and the interface said it did not. Its doc — and
// §13's published copy of it — read "maps option ids to names on export and
// names to ids on import", with OptionName named as the only method carrying
// a duty on both sides. `optionNameTaken` has asked OptionId from the export
// side since the avoid-set became the PROPERTY's options rather than one
// document's census: the rung-(b) term must not be a name some other option
// of the property already answers to, and the census cannot see an option
// this document never writes.
//
// The two directions ask it for different things, which is why the contract
// has to say both. Import wants the ID — and takes the FIRST of a duplicated
// name, which is one of the two losses `option_ids` exists to close. Export
// wants only whether an answer EXISTS, discards the id, and is therefore
// indifferent to the scan order that makes the import answer a hint.
func TestOptionRefs_ExportAsksOptionIdAboutTheTermItIsAboutToMint(t *testing.T) {
	// given — two options claiming one name, so a term must be minted
	rec := &recordingOptions{answerId: true, space: spaceOptions{"tag": {
		{id: "bafyfirst", name: "books"},
		{id: "bafysecond", name: "books"},
	}}}
	...
	// when
	data, err := Marshal(model.SmartBlockType_Page, snap, Options{ResolveOptions: rec})
	require.NoError(t, err)
	require.Equal(t, []any{"books (yfirst)", "books (second)"}, docProperty(t, data, "Tag"))

	// then — export asked the name→id direction about every term it was
	// about to write, which is the avoid-set question
	assert.Subset(t, rec.names, []string{"books (yfirst)", "books (second)"},
		"export must ask whether the property already names an option the term")

	// and the published contract says so, in both copies. The interface is
	// frozen: a consumer implementing it from §13 alone must know that a
	// stub OptionId costs something on the way OUT as well as the way in.
	spec := readFormatDocumentation(t)["SPEC.md"]
	assert.NotContains(t, spec, "names to ids on import (creating options is the import wiring's job)",
		"§13 said OptionId is the import half; export has asked it since the avoid-set widened")
	assert.Contains(t, spec, "Both methods are asked in both directions",
		"§13 must state the duty export has on OptionId")
}

// A resolver that names ids but cannot invert them is a legitimate shape, and
// the interface now says what it costs on the way OUT: the avoid-set goes
// quiet, so a minted term may be a name some live option of the property
// already answers to — the exact fault the avoid-set exists to prevent, and
// invisible in the document, because the legend entry beside it looks right.
func TestOptionRefs_AResolverThatCannotInvertLosesTheAvoidSet(t *testing.T) {
	// given — three options, the document on two, and the third already
	// named the term rung (b) wants for the first
	space := spaceOptions{"tag": {
		{id: "bafyfirst", name: "books"},
		{id: "bafysecond", name: "books"},
	...
	// when — the same document exported through a resolver that answers
	// OptionId and one that does not
	answering, err := Marshal(model.SmartBlockType_Page, snap,
		Options{ResolveOptions: &recordingOptions{space: space, answerId: true}})
	require.NoError(t, err)
	silent, err := Marshal(model.SmartBlockType_Page, snap,
		Options{ResolveOptions: &recordingOptions{space: space, answerId: false}})
	require.NoError(t, err)

	// then — the answering resolver sends the contested claimant to rung (c)
	assert.Equal(t, []any{"bafyfirst", "books (second)"}, docProperty(t, answering, "Tag"))

	// and the silent one mints the term the third option answers to, which is
	// what the interface doc now warns about rather than leaving to be found
	assert.Equal(t, []any{"books (yfirst)", "books (second)"}, docProperty(t, silent, "Tag"))
}
