...
func TestRelationEnvelope_RefusedInProperties(t *testing.T) {
	...
	for spelling, value := range map[string]string{
		...
		t.Run(spelling, func(t *testing.T) {
			doc := `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"budget",` +
...
func TestRelationEnvelope_WrongFormatWarnsButCarries(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"include_time true on number": {
			doc:      `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number","include_time":true}}`,
			wantWarn: "/property_settings/include_time",
		},
		"object_types non-empty on number": {
			doc:      `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number","object_types":["page"]}}`,
			wantWarn: "/property_settings/object_types",
		},
		"include_time false on number": {
			doc: `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number","include_time":false}}`,
		},
		"object_types empty on number": {
			doc: `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number","object_types":[]}}`,
		},
		"include_time true on date": {
			doc: `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"date","include_time":true}}`,
		},
		"object_types non-empty on objects": {
			doc: `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"objects","object_types":["page"]}}`,
		},
	} {
		t.Run(name, func(t *testing.T) {
			// when
			var warns []Issue
			err := Validate([]byte(tc.doc), Options{OnWarning: func(i Issue) { warns = append(warns, i) }})
...
func TestRelationEnvelope_PhantomFieldNameInPropertiesWarns(t *testing.T) {
	for member, value := range map[string]string{
		...
		t.Run(member, func(t *testing.T) {
			...
			doc := `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b",` +
				`"property_settings":{"format":"number"},` +
				`"properties":{"name":"Budget","` + member + `":` + value + `}}`

			// when
			var warns []Issue
			err := Validate([]byte(doc), Options{OnWarning: func(i Issue) { warns = append(warns, i) }})
			...
			warns = nil
			page := `{"formatVersion":"2.0","id":"o1","properties":{"` + member + `":` + value + `}}`
			require.NoError(t, Validate([]byte(page), Options{OnWarning: func(i Issue) { warns = append(warns, i) }}))
			assert.Empty(t, warns)
		})
	}
}
...
func TestRelationEnvelope_FieldsAreGatedByKind(t *testing.T) {
	for name, tc := range map[string]struct{ doc, want string }{
		"property_settings on a page": {
			doc:  `{"formatVersion":"2.0","id":"o1","property_settings":{"format":"number"}}`,
			want: `/property_settings: property "property_settings" is only valid on property documents`,
		},
		"property_settings on a template": {
			doc:  `{"formatVersion":"2.0","kind":"template","id":"o1","type":"template","property_settings":{"format":"date"}}`,
			want: `/property_settings: property "property_settings" is only valid on property documents`,
		},
		"property_settings on a type": {
			doc:  `{"formatVersion":"2.0","kind":"object_type","id":"o1","property_settings":{"format":"objects"}}`,
			want: `/property_settings: property "property_settings" is only valid on property documents`,
		},
		...
		"missing format inside the group": {
			doc:  `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{}}`,
			want: "/property_settings: missing property 'format'",
		},
		"the pre-v0.32 root spelling": {
			doc:  `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","format":"number"}`,
			want: "moved off the root",
		},
		"a refused member inside the group names its home": {
			doc:  `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","property_settings":{"format":"number","name":"Budget"}}`,
			want: "the property's name is the `name` property",
		},
		"legacy relation_format beside a missing definition": {
			doc:  `{"formatVersion":"2.0","kind":"property","id":"o1","internal_key":"b","properties":{"relation_format":100}}`,
...
func TestRelationEnvelope_TheSideDoorKindsAreGuardedToo(t *testing.T) {
	for kind, wantValid := range map[string]bool{
		...
		t.Run(kind, func(t *testing.T) {
			...
			doc := []byte(`{"formatVersion":"2.0","kind":"` + kind + `","internal_key":"eh",` +
...
func TestRelationEnvelope_TheMissingFormatVerdictNamesTheWrongContainer(t *testing.T) {
	t.Run("format written into properties", func(t *testing.T) {
		// given
		doc := []byte(`{"formatVersion":"2.0","kind":"property","internal_key":"eh",` +
			`"properties":{"name":"Estimated Hours","format":"number"}}`)
...
// went unnoticed. Heart's extraction-time recovery tool reads arbitrary pb backups, where a
...
func TestPropertyFormat_MapIsNotAuthorable(t *testing.T) {
	t.Run("a relation document may state it", func(t *testing.T) {
		doc := []byte(`{"formatVersion":"2.0","kind":"property","internal_key":"templatePlaceholders",
			"property_settings":{"format":"map"},
			"properties":{"name":"Template Placeholders"}}`)
		assert.NoError(t, Validate(doc, Options{}), "the only carrier of `map` must keep exporting")
	})

	t.Run("a type may not declare it", func(t *testing.T) {
		doc := []byte(`{"formatVersion":"2.0","kind":"object_type","internal_key":"task","properties":{"name":"Task"},
			"type_settings":{"property_definitions": [{"property":"placeholders","format":"map"}]}}`)
		require.Error(t, Validate(doc, Options{}), "an authored property may not invent a map")
	})

	t.Run("a dataview may not declare it", func(t *testing.T) {
		doc := []byte(`{"formatVersion":"2.0","blocks":[{"type":"dataview",
			"properties":[{"property":"placeholders","format":"map"}],
			"views":[{"name":"All"}]}]}`)
		require.Error(t, Validate(doc, Options{}), "a view may not invent a map either")
	})
	...
	t.Run("the authored slots still take every other format", func(t *testing.T) {
		for _, f := range []string{"text", "number", "date", "select", "objects", "properties"} {
			...
			assert.NoErrorf(t, Validate(doc, Options{}), "%q is authorable", f)
		}
	})
}
