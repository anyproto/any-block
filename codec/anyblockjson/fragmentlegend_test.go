...
// A fragment's supplied legend is part of the document it was cut from, not
// merely an import hint. It must pass the same admission rules as an envelope
// legend before any fragment surface uses it.
func TestFragmentEntryPoints_ValidateTheSuppliedLegend(t *testing.T) {
	opts := Options{Legend: Legend{PropertyKeys: map[string]string{"alias": "id"}}}
	assertLegendError := func(t *testing.T, err error) {
		t.Helper()
	...
	t.Run("block run", func(t *testing.T) {
		_, _, err := UnmarshalBlocks(rawRun(
			`{"type":"link","object_id":"target","properties":["alias"]}`), opts)
		assertLegendError(t, err)
	})
	...
	t.Run("filters", func(t *testing.T) {
		_, err := UnmarshalFilters(json.RawMessage(
			`[{"property":"alias","condition":"equal","value":"x"}]`), opts)
		assertLegendError(t, err)
	})
	t.Run("sorts", func(t *testing.T) {
		_, err := UnmarshalSorts(json.RawMessage(`[{"property":"alias"}]`), opts)
		assertLegendError(t, err)
	})
}
