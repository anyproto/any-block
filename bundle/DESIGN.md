...
Status: IMPLEMENTED (2026-08-26). §1's pipeline is live:
collection behind Heart's `core/block/export/collect` (Closure replacing
`isProtobuf`), composition in this repository's root `bundle` package (the
extracted form of Q10 option b), the exporter wiring in Heart's
`core/block/export/anyblock`, and the manifest `files` map as
`../format/v2/SPEC.md` §2c
...
gated at the call site on the any-sync release whose object cache refuses
to close a still-loading entry (anyblock.go). Q6 landed as its
recommendation (a): `Export_AnyBlockV2 = 6` on `model.Export.Format`,
...
This file preserves the original architecture record and corpus evidence.
Implementation paths cited here that are **not rooted in this Go module** —
every package path outside `github.com/anyproto/any-block`, `core/…`,
`pkg/lib/…` and `cmd/internal/…` among them, and the historical `anyblock*`
conversion, batch and sweep tools — refer to the Anytype application, not to
this repository. Proposed snippets below are historical rationale, not the
public API; the current API is the source in this directory and `README.md`.
...
`UsedPropertyKeysFromBytes` in `codec/anyblockjson` or a small exported
...
Authored bundles (consumer 2, SPEC §2c) are unconstrained by all of
...
```
files/bafyreicccccccccccccccccccccccccccccccccccccccccccccccccccc.anyblock.json  ← the document
files/bafyreicccccccccccccccccccccccccccccccccccccccccccccccccccc.png            ← the bytes
```
...
SPEC §2c says the format refuses to define); the blob's extension must
...
or id rather than by walking" (SPEC §2c) — gains a third
...
any layout it likes, which is what makes consumer 2 (SPEC §2c) a
...
Evicting an object-cache entry that is still LOADING was not safe: the
close path assumes a loaded value, so an entry caught mid-load could be
left wedged rather than evicted. The exporter's `TryRemoveFromCache` is
issued only after its own `cache.Do` has returned, so the entry it evicts
is loaded — close-after-write is defined as closing what this task itself
finished loading, never "close whatever is there". The one ordering the
exporter cannot impose is another caller (UI open, sync) re-loading the
same object concurrently, and that is handled like the failure modes
above: the close is declined or fails, it is logged rather than retried,
and the passive TTL collects the entry later. Weighed in Q11, which also
gates the shipped path on the any-sync release that makes the cache refuse
such a close outright.
...
explicitly refuses to define layout (SPEC §2c), leaves authored bundles
...
alternative, and the reasoning that decided it. Q4-Q6 and Q8-Q11 were
...
**Settled as (a), and shipped: (b) leaves consumer 2 without free layout
and makes convention load-bearing; (c) is drift by construction.**
...
Shipped as (a) (2026-08-27): `Export_AnyBlockV2 = 6` on
...
round-trip soak matches the sweep's 99.98%; single-object in-memory export
emits exactly one document, no bundle files, per PRINCIPLES rule 7 ("a
document stands alone") — the last of which shipped.**

**Q8. Extension: `.anyblock.json` or bare `.json`? — SETTLED:
`.anyblock.json`.**
Why it mattered: §15 #1 then leaned toward bare
`.json`, but the bundle legitimately contains blobs that are themselves
`.json` files (12 corpus file objects have `file_ext == "json"`), and
...
principle. **Settled as (a) for the default mode, via the per-bundle
`BundleRoot` prefix; readable space directories ride the human mode when it
is designed. (b) only if a real consumer materializes — each per-space
`index.json` already carries the space's own name, so a tool can build the
listing in one pass.**
...
`codec/anyblockjson`; (b) a new `codec/anyblockjson/compose`
...
**Resolution:** option (b), extracted as the repository-root `bundle`
package rather than the proposed `codec/anyblockjson/compose` subpackage.

**Q11. Does close-after-write need the object cache's mid-load close made
safe first? — SETTLED: yes, and it is gated on that release.**
Why it mattered: the memory model (§1.6) stands on the emit task calling
`TryRemoveFromCache` after every write (§1.5), and closing a cache entry
that was still loading was not safe (§1.5). The exporter would be the
heaviest caller of that path in the application. Options: (a) depend on
the any-sync release that makes the cache refuse to close a still-loading
entry; (b) ship close-after-write regardless, on the strength of the
exporter's own ordering — its call lands after its own `cache.Do` returns,
so the entry is loaded, and only a concurrent re-load by another caller
could catch one mid-load; (c) TTL-only — rejected by the memory model
itself (§1.6: the passive window is throughput-proportional and unbounded
by space size). **Settled as (a):** an export that can wedge a cache entry
is a worse failure than the memory peak it prevents, so the shipped
exporter names that release as its floor at the call site rather than
relying on (b)'s ordering alone.
...
**Historical proposed interfaces** (outline only — signatures, no bodies;
not the extracted repository API):
...
```go
// historical proposal; implemented as repository-root package bundle
package compose
