...
Status: living document · applies to `formatVersion` 2.0 (SPEC draft) ·
Package: `codec/anyblockjson`
...
serve or knowingly bend. The TL;DR is all of it on one screen; `README.md`
walks through the individual decisions.
...
1. **Lossless for meaning.** What the user expressed survives a round trip;
   ...
   written down.
2. **Readable by a stranger.** A person who has never seen Anytype
   internals can read a document, understand it, and hand-edit it.
...
4. **Nothing to guess.** A valid document needs only what is in the
   author's head and in one example — no offsets, no ids to fetch first, no
...
10. **Evolution is explicit.** One `major.minor` `formatVersion`; a reader
    refuses what it does not know; every grammar change is a bump.

When they collide: the user's meaning survives › a model (a small one too)
can write it › a stranger can read it › it costs few tokens › it is
convenient to implement or faithful to an internal name. Two things never
yield: `Marshal` never emits what `Validate` rejects, and no loss is
silent.

Not: a sync or wire format · a replacement for the Markdown export ·
byte-equal to arbitrary input · forward-compatible.

A change to the format, to `codec/anyblockjson`, or to API v2's document surface
names the rule it serves — or the rule it bends and the cost it accepts.
Rules move on evidence, never on taste.

What follows is the shape, then the rationale and the evidence. `SPEC.md`
...
One JSON document per Anytype object: an envelope (`formatVersion`, `id`, `type`,
...
subset inside `text`. Types are documents too (`kind: "object_type"` with a
`type_settings` group carrying `property_definitions`); a bundle of documents
adds an `index.json`.
...
select options by name, formatting inline:
...
```json
{
  ...
  "blocks": [
    ...
    { "type": "paragraph",
      "text": "Lossless **and** readable, with <mention object_id=\"bafyreidfmzjh…\">Alice</mention>." },
...
regenerates, restrictions it rebuilds, document-local ids, offsets, cached formats, UI
...
`null` is a fact the user created and is written verbatim, bar seven
system-stamped keys whose empty value is their semantic default; the
omit-default canon applies to block attributes and envelope fields only
...
text, not marks, block order, tables or dataviews — so a pass rate is a
...
`snake_case`, digits included, except the conventional envelope metadata key
`formatVersion`; the rule is stated so a domain name added later needs no
decision (§1 *Naming*). So `relation` → `property`, `smartBlockType` →
...
- Document-local block/table/view ids are optional and minted on import
  (§9); a provided envelope object id is preserved, including under
  `OmitIds`. Numbering, structural blocks and restrictions are derived, never
  written (§5, §7).
...
rebind (§8.1, §9a, §10). `Validate` and strict
`Unmarshal(data, Options{})` accept and reject the same documents under the
resolver-free, default bundled vocabulary, and `Marshal` never emits what
`Validate` rejects (§11, §12). A wider or store-backed `Options.Keys` may
resolve additional spellings and add semantic refusals — denied or
unwritable resolved keys, ambiguities, or binding collisions — that byte-only
`Validate` cannot predict; every byte-level rejection still reaches import
because validation runs first. Thus "this document imports under the default
vocabulary" is a statement rather than a hope. And a check has to earn its place: it catches something silent *and*
traces to a mechanism, or it does not ship — every marginal warning makes
the ones that matter cheaper to ignore (§12).

Absent `kind` means page regardless of the `type` spelling, so current
`formatVersion: "2.0"` accepts kindless `"type": "template"` as a page whose
type term resolves normally. Actual templates state `kind: "template"`.
Pre-freeze drafts that gave the kindless spelling template semantics declare
`"version": 1` and are refused by the version gate, not by reserving a type
name (§2, §10, §15 #9).
...
**One public `major.minor` format version. A reader refuses what it does not
know, migrates only what it explicitly supports, and every grammar change is
a bump.**

`formatVersion` is the sole authority on format identity (§10). A reader rejects
a newer document with a dedicated error naming both versions, accepts an
older one only through a defined migration, and every grammar change bumps —
a closed schema has nothing additive to offer an older reader anyway. Inside `text`,
...
did: *presence is meaningful* (meaning survives, over token cost — 14,032
omitted defaults put back); *flat blocks* (a model can write it, over a
...
- Package tests: golden files, property-based round-trip tests over
  ...
  agree under the resolver-free, default bundled vocabulary (§11, §12).
  A wider/store-backed `Options.Keys` may narrow import acceptance only with
  path-addressed semantic refusals; every byte-validation rejection still
  reaches import. A round-trip tool in the Anytype application sweeps a
  production account.
...
- A change to the format, to this package, or to API v2's document surface
  names the rule it serves — or the rule it bends and the cost it accepts,
  recorded in the commit that makes it.
