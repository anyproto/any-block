...
| [`exported_space/`](exported_space) | a tiny **export-shaped** bundle: stored keys, a document legend, published enum names, a key nothing could define, a reference that resolves and one that does not, and an image block that walks through a file document and `manifest.files` to a real blob. Synthetic, and what the reader runs on. |
