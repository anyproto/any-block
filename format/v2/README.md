...
split has one name between them, `text`. Every domain identifier the format
defines is spelled `snake_case`, digits included; the envelope metadata key
`formatVersion` is the sole format-owned camelCase exception. Property and
type keys are also exempt because they name things a user named and spell the
display name raw (decision 3).
...
The format was swept over a real production account — every object exported to
AnyBlock JSON, re-imported, and re-exported, comparing both the state and the
bytes. The harness was a round-trip tool in the Anytype application.
...
test had. That is the reason to check the round-trip
...
- **No shipped v2 compatibility burden before 2.0.** The only pre-release
  migration is mechanical: readers normalize integer `version: 2` to public
  `formatVersion: "2.0"`; ambiguous `version: 1` drafts are refused.
